...
public class MatingRecipe implements IRecipe {
	...
	public MatingRecipe() {
		unknown = PluginLepidopterology.butterflyInterface.getMemberStack(
				PluginLepidopterology.butterflyInterface.getIndividualTemplates().get(0),
	...
	public boolean matches(InventoryCrafting crafting, World world) {
		...
		for (int i = 0; i < crafting.getSizeInventory(); i++) {
			if (PluginLepidopterology.butterflyInterface.isMember(crafting.getStackInSlot(i),
					EnumFlutterType.BUTTERFLY.ordinal())) {
				butterflies++;
				mated = PluginLepidopterology.butterflyInterface.isMated(crafting.getStackInSlot(i));
				cached = crafting.getStackInSlot(i);
			} else if (PluginLepidopterology.butterflyInterface.isMember(crafting.getStackInSlot(i),
