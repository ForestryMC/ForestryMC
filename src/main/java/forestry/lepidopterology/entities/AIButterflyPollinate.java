...
public class AIButterflyPollinate extends AIButterflyInteract {
	...
	protected boolean canInteract() {
		...
		if (!entity.getButterfly().getGenome().getFlowerProvider().isAcceptedPollinatable(entity.world, checkPollinatable)) {
			return false;
		}
		...
		if (continueExecuting() && rest != null) {
			...
			if (checkPollinatable != null) {
				if (entity.getPollen() == null) {
					...
					IPollinatable realPollinatable = GeneticsUtil.getOrCreatePollinatable(null, entity.world, rest);
