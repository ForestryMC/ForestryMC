...
public class AIButterflyPollinate extends AIButterflyInteract {
	...
	protected boolean canInteract() {
		...
		if (!entity.getButterfly().getGenome().getFlowerProvider().isAcceptedPollinatable(entity.worldObj,
				new FakePollinatable(checkPollinatable))) {
			return false;
		}
		...
		if (continueExecuting()) {
			...
			if (checkPollinatable != null) {
				if (entity.getPollen() == null) {
					...
					// Proxies.log.finest("A butterfly '%s' grabbed a pollen
					// '%s' at %s/%s/%s.", entity.getButterfly().getIdent(),
					// entity.getPollen().getIdent(), rest.posX, rest.posY,
					// rest.posZ);
					...
					if (realPollinatable != null) {
						...
						// Proxies.log.finest("A butterfly '%s' unloaded pollen
						// '%s' at %s/%s/%s.", entity.getButterfly().getIdent(),
						// entity.getPollen().getIdent(), rest.posX, rest.posY,
						// rest.posZ);
