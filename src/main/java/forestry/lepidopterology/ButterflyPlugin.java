...
public class ButterflyPlugin implements IGeneticPlugin {
	...
	public void createRoot(IRootManager rootManager, IGeneticFactory geneticFactory) {
		...
		rootBuilder.setRootFactory(ButterflyRoot::new).setSpeciesType(ButterflyChromosomes.SPECIES).addListener(ComponentKeys.TYPES, (IOrganismTypes<IButterfly> builder) -> {
		...
		}).addComponent(ComponentKeys.TRANSLATORS).addComponent(ComponentKeys.MUTATIONS).addComponent(ForestryComponentKeys.RESEARCH, ResearchHandler::new).addListener(ForestryComponentKeys.RESEARCH, (IResearchHandler<IButterfly> component) -> {
