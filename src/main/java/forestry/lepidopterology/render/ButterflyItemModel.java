...
public class ButterflyItemModel extends AbstractBakedModel {
	...
	private static class Geometry implements IModelGeometry<Geometry> {
		...
		public IBakedModel bake(IModelConfiguration owner, ModelBakery bakery, Function<RenderMaterial, TextureAtlasSprite> spriteGetter, IModelTransform modelTransform, ItemOverrideList overrides, ResourceLocation modelLocation) {
			...
			for (Map.Entry<String, String> subModel : this.subModels.entrySet()) {
				...
				BlockModel model = new BlockModel(modelBlock.getParentLocation(), modelBlock.getElements(), ImmutableMap.of("butterfly", Either.left(new RenderMaterial(PlayerContainer.LOCATION_BLOCKS_TEXTURE, new ResourceLocation(texture)))), modelBlock.ambientOcclusion, modelBlock.getGuiLight(), modelBlock.getAllTransforms(), modelBlock.getOverrides());
	...
	private class OverrideList extends ItemOverrideList {
		...
		private ImmutableMap<ItemCameraTransforms.TransformType, TransformationMatrix> getTransformations(float size) {
			...
			builder.put(ItemCameraTransforms.TransformType.FIXED, new TransformationMatrix(new Vector3f(scale * 0.5F, scale - (size / 0.75F) * scale, scale * 1.25F), null, scaledSize, null));
			builder.put(ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, new TransformationMatrix(new Vector3f(0, -scale * 4.75F, 0), null, scaledSize, null));
			builder.put(ItemCameraTransforms.TransformType.THIRD_PERSON_LEFT_HAND, new TransformationMatrix(new Vector3f(0, -scale * 4.75F, 0), null, scaledSize, null));
			builder.put(ItemCameraTransforms.TransformType.GUI, new TransformationMatrix(new Vector3f(0, -scale, 0), new Quaternion(new Vector3f(1, 0, 0), 90F, true), scaledSize, null));
			builder.put(ItemCameraTransforms.TransformType.FIRST_PERSON_LEFT_HAND, new TransformationMatrix(new Vector3f(0, 0, 0), null, scaledSize, null));
