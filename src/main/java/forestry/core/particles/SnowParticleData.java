...
public class SnowParticleData implements IParticleData {
	...
	public static final IDeserializer<SnowParticleData> DESERIALIZER = new IDeserializer<SnowParticleData>() {
		@Nonnull
		@Override
		public SnowParticleData deserialize(@Nonnull ParticleType<SnowParticleData> type, @Nonnull StringReader reader) throws CommandSyntaxException {
			reader.expect(' ');
	...
	public static final Codec<SnowParticleData> CODEC = RecordCodecBuilder.create(val -> val.group(Codec.DOUBLE.fieldOf("particleStart").forGetter(data -> data.particleStart.getX()), Codec.DOUBLE.fieldOf("particleStart").forGetter(data -> data.particleStart.getY()), Codec.DOUBLE.fieldOf("particleStart").forGetter(data -> data.particleStart.getY())).apply(val, SnowParticleData::new));
	...
	public String getParameters() {
		return String.format(Locale.ROOT, "%s %2f %2f %2f", getType().getRegistryName(), particleStart.getX(), particleStart.getY(), particleStart.getZ());
	}
}
