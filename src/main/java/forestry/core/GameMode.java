...
public class GameMode implements IGameMode {
	...
	public GameMode(String identifier) {
		...
		if (oldConfigFile.exists()) {
			...
			if (oldConfigFile.renameTo(oldConfigFileRenamed)) {
				Proxies.log.info("Migrated " + category + " settings to the new file '" + newConfig + "' and renamed '"
						+ oldConfig + "' to '" + oldConfigRenamed + "'.");
			}
		}
	...
	private void loadOldConfig() {
		initSettingFloat_old("energy.demand.modifier", ENERGY_DEMAND_MODIFIER, -1,
				"modifies the energy required to activate machines, as well as the max amount of energy stored and accepted.");
		initSettingInt_old("farms.fertilizer.value", FARM_FERTILIZER_VALUE, -1,
				"modifies the time a piece of fertilizer lasts in a farm.");

		initSettingFloat_old("fuel.ethanol.generator", FUEL_MODIFIER, -1,
				"modifies the energy provided by ethanol in a Bio Generator.");
		initSettingFloat_old("fuel.ethanol.combustion", FUEL_MODIFIER, -1,
				"modifies the energy provided by ethanol in Buildcraft Combustion Engines.");
		initSettingFloat_old("fuel.biomass.generator", FUEL_MODIFIER, -1,
				"modifies the energy provided by Biomass in a Bio Generator.");
		initSettingFloat_old("fuel.biomass.biogas", FUEL_MODIFIER, -1,
				"modifies the energy provided by Biomass in Biogas Engines.");

		initSettingStack_old("recipe.output.fertilizer.apatite", recipeFertilizerOutputApatite,
				"amount of fertilizer yielded by the recipe using apatite.");
		initSettingStack_old("recipe.output.fertilizer.ash", recipeFertilizerOutputAsh,
				"amount of fertilizer yielded by the recipe using ash.");
		initSettingStack_old("recipe.output.compost.wheat", recipeCompostOutputWheat,
				"amount of compost yielded by the recipe using wheat.");
		initSettingStack_old("recipe.output.compost.ash", recipeCompostOutputAsh,
				"amount of compost yielded by the recipe using ash.");
		initSettingStack_old("recipe.output.humus.fertilizer", recipeHumusOutputFertilizer,
				"amount of humus yielded by the recipe using fertilizer.");
		initSettingStack_old("recipe.output.humus.compost", recipeHumusOutputCompost,
				"amount of humus yielded by the recipe using compost.");
		initSettingStack_old("recipe.output.bogearth.bucket", recipeBogEarthOutputBucket,
				"amount of bog earth yielded by the recipe using buckets.");
		initSettingStack_old("recipe.output.bogearth.can", recipeBogEarthOutputCans,
		...
		initSettingStack_old("recipe.output.capsule", recipeCapsuleOutput,
				"amount yielded by the recipe for wax capsules.");
		initSettingStack_old("recipe.output.refractory", recipeRefractoryOutput,
				"amount yielded by the recipe for refractory capsules.");

		initSettingInt_old("fermenter.cycles.fertilizer", FERMENTATION_DURATION_FERTILIZER, -1,
				"modifies the amount of cycles fertilizer can keep a fermenter going.");
		initSettingInt_old("fermenter.cycles.compost", FERMENTATION_DURATION_COMPOST, -1,
				"modifies the amount of cycles compost can keep a fermenter going.");

		initSettingInt_old("fermenter.value.fertilizer", FERMENTED_CYCLE_FERTILIZER, -1,
				"modifies the amount of biomass per cycle a fermenter will produce using fertilizer.");
		initSettingInt_old("fermenter.value.compost", FERMENTED_CYCLE_COMPOST, -1,
				"modifies the amount of biomass per cycle a fermenter will produce using compost.");

		initSettingInt_old("fermenter.yield.sapling", FERMENTED_SAPLING, FERMENTED_SAPLING * 8,
				"modifies the base amount of biomass a sapling will yield in a fermenter, affected by sappiness trait.");
		initSettingInt_old("fermenter.yield.cactus", FERMENTED_CACTI, FERMENTED_CACTI * 8,
				"modifies the amount of biomass a piece of cactus will yield in a fermenter.");
		initSettingInt_old("fermenter.yield.wheat", FERMENTED_WHEAT, FERMENTED_WHEAT * 8,
				"modifies the amount of biomass a piece of wheat will yield in a fermenter.");
		initSettingInt_old("fermenter.yield.cane", FERMENTED_CANE, FERMENTED_CANE * 8,
				"modifies the amount of biomass a piece of sugar cane will yield in a fermenter.");
		initSettingInt_old("fermenter.yield.mushroom", FERMENTED_MUSHROOM, FERMENTED_MUSHROOM * 8,
				"modifies the amount of biomass a mushroom will yield in a fermenter.");

		initSettingInt_old("squeezer.liquid.seed", SQUEEZED_LIQUID_SEED, SQUEEZED_LIQUID_SEED * 8,
				"modifies the amount of seed oil squeezed from a single seed. other sources are based off this.");
		initSettingInt_old("squeezer.liquid.apple", SQUEEZED_LIQUID_APPLE, SQUEEZED_LIQUID_APPLE * 8,
				"modifies the amount of juice squeezed from a single apple. other sources are based off this.");
		initSettingInt_old("squeezer.mulch.apple", SQUEEZED_MULCH_APPLE, SQUEEZED_MULCH_APPLE * 8,
