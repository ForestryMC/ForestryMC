...
public abstract class GuiAlyzer extends GuiForestry<ContainerAlyzer, IInventory> {
	...
	protected GuiAlyzer(String speciesRoot, EntityPlayer player, ContainerAlyzer container, IInventory inventory,
			String guiName) {
	...
	protected final void drawRow(String text0, String text1, String text2, IIndividual individual,
		...
		drawRow(text0, text1, text2, fontColor.get("gui.screen"),
				getColorCoding(individual.getGenome().getActiveAllele(chromosome).isDominant()),
				getColorCoding(individual.getGenome().getInactiveAllele(chromosome).isDominant()));
	}
	...
	protected final void drawChromosomeRow(String chromosomeName, IIndividual individual, IChromosomeType chromosome) {
		...
		drawRow(chromosomeName, active.getName(), inactive.getName(), fontColor.get("gui.screen"),
				getColorCoding(active.isDominant()), getColorCoding(inactive.isDominant()));
	}
	...
	protected void drawAnalyticsOverview() {
		...
		fontRendererObj.drawSplitString(StringUtil.localize(guiName + ".help"), guiLeft + COLUMN_0 + 4, guiTop + 42,
				200, fontColor.get("gui.screen"));
		newLine();
		newLine();
		newLine();
	...
	protected final void drawAnalyticsPageClassification(IIndividual individual) {
		...
		if (StringUtils.isBlank(description) || description.startsWith("for.description.")) {
			...
			if (tokens.length > 1) {
				...
				fontRendererObj.drawStringWithShadow(signature,
						(guiLeft + 210) - fontRendererObj.getStringWidth(signature), guiTop + 145 - 14, 0x99cc32);
			}
		}
