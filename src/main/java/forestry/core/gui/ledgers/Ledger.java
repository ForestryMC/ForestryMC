...
public abstract class Ledger {
	...
	private static final ResourceLocation ledgerTextureRight = new ForestryResource(
			Defaults.TEXTURE_PATH_GUI + "/ledger.png");
	private static final ResourceLocation ledgerTextureLeft = new ForestryResource(
			Defaults.TEXTURE_PATH_GUI + "/ledgerLeft.png");
	...
	// adjust the update's move amount to match the look of 60 fps (16.67 ms per
	// update)
	...
	protected void drawBackground(int x, int y) {
		...
		manager.gui.drawTexturedModalRect(x, y + 4, 0, 256 - height + 4, 4, height - 4); // left
																							// edge
		manager.gui.drawTexturedModalRect(x + 4, y, 256 - width + 4, 0, width - 4, 4); // top
																						// edge
	...
	protected void drawSprite(ISprite icon, int x, int y) {

		if (icon != null) {
			GL11.glDisable(GL11.GL_LIGHTING);
			GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0F);
			Proxies.common.bindTexture();
			manager.gui.drawTexturedModelRect(x, y, icon, 16, 16);
	...
	protected void drawHeader(String string, int x, int y) {
		...
		minecraft.fontRendererObj.drawStringWithShadow(string, x, y, fontColorHeader);
	}

	protected void drawSubheader(String string, int x, int y) {
		Minecraft minecraft = Proxies.common.getClientInstance();
		minecraft.fontRendererObj.drawStringWithShadow(string, x, y, fontColorSubheader);
	}
	...
	protected void drawSplitText(String string, int x, int y, int width) {
		Minecraft minecraft = Proxies.common.getClientInstance();
		minecraft.fontRendererObj.drawSplitString(string, x, y, width, fontColorText);
	}
}
