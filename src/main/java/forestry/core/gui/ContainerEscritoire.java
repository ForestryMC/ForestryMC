...
public class ContainerEscritoire extends ContainerTile<TileEscritoire> implements IGuiSelectable {
	...
	public ContainerEscritoire(EntityPlayer player, TileEscritoire tile) {
		...
		for (int i = 0; i < TileEscritoire.SLOTS_INPUT_COUNT; i++) {
			addSlotToContainer(new SlotFiltered(this.tile, TileEscritoire.SLOT_INPUT_1 + i, 17, 49 + i * 18)
					.setBlockedTexture("slots/blocked_2"));
		}

		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 2; j++) {
				addSlotToContainer(new SlotOutput(this.tile, TileEscritoire.SLOT_RESULTS_1 + (i * 2) + j, 177 + j * 18,
						85 + i * 18));
			}
		}
	}
