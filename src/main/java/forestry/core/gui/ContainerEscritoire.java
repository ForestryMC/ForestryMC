...
public class ContainerEscritoire extends ContainerTile<TileEscritoire> implements IGuiSelectable {
    ...
    public ContainerEscritoire(EntityPlayer player, TileEscritoire tile) {
        ...
        // Analyze slot
        addSlotToContainer(
                new SlotFiltered(this.tile, InventoryEscritoire.SLOT_ANALYZE, 97, 67).setPickupWatcher(this.tile)
                        .setStackLimit(1));

        for (int i = 0; i < InventoryEscritoire.SLOTS_INPUT_COUNT; i++) {
            addSlotToContainer(
                    new SlotFiltered(this.tile, InventoryEscritoire.SLOT_INPUT_1 + i, 17, 49 + i * 18)
