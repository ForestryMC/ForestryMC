...
class TableTitle extends TableText {
	public TableTitle(String text) {
		super(text);
	}
	...
	public void draw(FontRenderer fontRenderer, int x, int y, int lineWidth, int lineStart, int lineEnd, int rowTopY, int rowBottomY, int fontColor, boolean drawBackground) {
		if(drawBackground) {
			Gui.drawRect(x, rowBottomY - 1, lineEnd, y, TITLE_BACKGROUND);
			Gui.drawRect(x, rowBottomY, lineEnd, rowBottomY - 1, DEFAULT_BACKGROUND);
		}
