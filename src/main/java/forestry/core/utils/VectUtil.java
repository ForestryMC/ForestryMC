	...
	private static class MutableBlockPosSpiralIterator extends AbstractIterator<BlockPos.Mutable> {
		...
		protected BlockPos.Mutable nextPos() {
			if (this.theBlockPos == null) {
				...
				int y = Math.min(this.maxPos.getY(), this.world.getHeightmapPos(Heightmap.Type.WORLD_SURFACE, this.theBlockPos).getY());
				...
				if (y > minPos.getY() && y > 0) {
					...
					y = Math.min(this.maxPos.getY(), this.world.getHeightmapPos(Heightmap.Type.WORLD_SURFACE, this.theBlockPos).getY());
				}
