...
public class RecipeUtil {
	...
	public static void injectLeveledRecipe(ItemStack resource, int fermentationValue, Fluids output) {
		...
		RecipeManagers.fermenterManager.addRecipe(resource, fermentationValue, 1.0f, output.getFluid(1),
				Fluids.WATER.getFluid(1));

		if (FluidRegistry.isFluidRegistered(Fluids.JUICE.getFluid())) {
			RecipeManagers.fermenterManager.addRecipe(resource, fermentationValue, 1.5f, output.getFluid(1),
					Fluids.JUICE.getFluid(1));
		}
	...
	/**
	 * Returns a list of the ore dictionary names if they exist. Returns a list
	 * containing itemStack if there are no ore dictionary names. Used for
	 * creating recipes that should accept equivalent itemStacks, based on the
	 * ore dictionary.
	 */
	...
	public static Object[] getCraftingRecipeAsArray(Object rec) {
		...
		try {
			...
			if (rec instanceof IDescriptiveRecipe) {
				...
				return getShapedRecipeAsArray(recipe.getWidth(), recipe.getHeight(), recipe.getIngredients(),
	...
	public static boolean canCraftRecipe(World world, ItemStack[] recipeItems, ItemStack recipeOutput,
			ItemStack[] availableItems) {
		...
		// Strategy:
		// Create a fake crafting inventory using items we have in
		// availableItems
		...
		for (int slot = 0; slot < recipeItems.length; slot++) {
			...
			for (ItemStack stockStack : stockCopy) {
				if (stockStack.stackSize > 0
						&& StackUtils.isCraftingEquivalent(recipeStack, stockStack, false, false)) {
			...
			if (crafting.getStackInSlot(slot) == null) {
				for (ItemStack stockStack : stockCopy) {
					if (stockStack.stackSize > 0
							&& StackUtils.isCraftingEquivalent(recipeStack, stockStack, true, true)) {
						ItemStack stack = StackUtils.createSplitStack(stockStack, 1);
						stockStack.stackSize--;
						crafting.setInventorySlotContents(slot, stack);
						break;
	...
	private static ItemStack findRepairRecipe(InventoryCrafting inventory) {
		...
		if (craftIngredientCount == 2 && itemstack0.getItem() == itemstack1.getItem() && itemstack0.stackSize == 1
				&& itemstack1.stackSize == 1 && itemstack0.getItem().isRepairable()) {
