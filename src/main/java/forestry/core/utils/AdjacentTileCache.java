...
public final class AdjacentTileCache {
	...
	private TileEntity searchSide(EnumFacing side) {
		...
		int sx = source.getPos().getX() + side.getFrontOffsetX();
	...
	private boolean areCoordinatesOnSide(EnumFacing side, TileEntity target) {
		return source.getPos().getX() + side.getFrontOffsetX() == target.getPos().getX()
				&& source.getPos().getY() + side.getFrontOffsetY() == target.getPos().getY()
				&& source.getPos().getZ() + side.getFrontOffsetZ() == target.getPos().getZ();
	}
