...
public class GuiSolderingIron extends GuiForestry<ContainerSolderingIron, SolderingInventory> {
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int mouseX, int mouseY) {
		...
		fontRendererObj.drawString(title, guiLeft + 8 + getCenteredOffset(title, 138), guiTop + 16,
				fontColor.get("gui.screen"));

		for (int l = 2; l < inventory.getSizeInventory(); l++) {
			String description;
			CircuitRecipe recipe = ItemSolderingIron.SolderManager.getMatchingRecipe(layout,
