...
public abstract class GuiHandlerBase implements IGuiHandler {
	...
	protected static <T extends TileEntity> T getTile(World world, BlockPos pos, EntityPlayer player, Class<T> tileClass) {
	...
	protected GuiNaturalistInventory getNaturalistChestGui(String rootUID, EntityPlayer player, World world, BlockPos pos, int page) {
	...
	protected ContainerNaturalistInventory getNaturalistChestContainer(String rootUID, EntityPlayer player, World world, BlockPos pos, int page) {
		ISpeciesRoot speciesRoot = AlleleManager.alleleRegistry.getSpeciesRoot(rootUID);
		speciesRoot.getBreedingTracker(world, player.getGameProfile()).synchToPlayer(player);
		return new ContainerNaturalistInventory(player.inventory, getTile(world, pos, player, TileNaturalistChest.class), page);
	}
