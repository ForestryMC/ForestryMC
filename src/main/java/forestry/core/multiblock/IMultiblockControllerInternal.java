...
public interface IMultiblockControllerInternal
        extends IMultiblockController, INBTTagable, IRestrictedAccess, IErrorLogicSource, IClimatised, IStreamableGui {
    ...
    /**
     * Call to detach a block from this machine. Generally, this should be called when the tile entity is being
     * released, e.g. on block destruction.
     * 
    ...
    /**
     * Check if the machine is whole or not. If the machine was not whole, but now is, assemble the machine. If the
     * machine was whole, but no longer is, disassemble the machine.
     */
    void checkIfMachineIsWhole();

    /**
     * Assimilate another controller into this controller. Acquire all of the other controller's blocks and attach them
     * to this one.
     *
    ...
    /**
     * Tests whether this multiblock should consume the other multiblock and become the new multiblock master when the
     * two multiblocks are adjacent. Assumes both multiblocks are the same type.
     * 
    ...
    /**
     * Called when this machine may need to check for blocks that are no longer physically connected to the reference
     * coordinate.
     * 
