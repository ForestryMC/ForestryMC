...
public abstract class MultiblockTileEntityBase extends IMultiblockPart implements ISidedInventory, IFilterSlotDelegate, IOwnable {
	...
	public final void assertDetached() {
		if (this.controller != null) {
			Proxies.log.info("[assert] Part @ (%d, %d, %d) should be detached already, but detected that it was not. This is not a fatal error, and will be repaired, but is unusual.", pos.getX(), pos.getY(), pos.getZ());
	...
	public final Packet getDescriptionPacket() {
		...
		return new S35PacketUpdateTileEntity(pos, 0, packetData);
	}
	...
	public final CoordTriplet getWorldLocation() {
		return new CoordTriplet(this.pos);
	}
	...
	public final IMultiblockPart[] getNeighboringParts() {
		CoordTriplet[] neighbors = new CoordTriplet[]{
				new CoordTriplet(pos.getX() - 1, pos.getY(), pos.getZ()),
				new CoordTriplet(pos.getX(), pos.getY() - 1, pos.getZ()),
				new CoordTriplet(pos.getX(), pos.getY(), pos.getZ() - 1),
				new CoordTriplet(pos.getX(), pos.getY(), pos.getZ() + 1),
				new CoordTriplet(pos.getX(), pos.getY() + 1, pos.getZ()),
				new CoordTriplet(pos.getX() + 1, pos.getY(), pos.getZ())
		};
		...
		for (CoordTriplet neighbor : neighbors) {
			...
			te = this.worldObj.getTileEntity(neighbor.pos);
