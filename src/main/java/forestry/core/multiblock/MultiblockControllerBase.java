...
public abstract class MultiblockControllerBase implements ISidedInventory, IRestrictedAccessTile, IErrorLogicSource {
	...
	public void attachBlock(IMultiblockPart part) {
		...
		if (this.referenceCoord == null) {
			...
			BlockPos pos = new BlockPos(referenceCoord.pos);
		...
		if (minimumCoord != null) {
			int x = minimumCoord.pos.getX();
			int y = minimumCoord.pos.getX();
			int z = minimumCoord.pos.getX();
			if (part.getPos().getX() < minimumCoord.pos.getX()) {
				x = part.getPos().getX();
			}
			if (part.getPos().getY() < minimumCoord.pos.getY()) {
				y = part.getPos().getY();
			}
			if (part.getPos().getZ() < minimumCoord.pos.getZ()) {
				z = part.getPos().getZ();
			}
			minimumCoord.pos = new BlockPos(x, y, z);
		}

		if (maximumCoord != null) {
			int x = maximumCoord.pos.getX();
			int y = maximumCoord.pos.getX();
			int z = maximumCoord.pos.getX();
			if (part.getPos().getX() > maximumCoord.pos.getX()) {
				x = part.getPos().getX();
			}
			if (part.getPos().getY() > maximumCoord.pos.getY()) {
				y = part.getPos().getY();
			}
			if (part.getPos().getZ() > maximumCoord.pos.getZ()) {
				z = part.getPos().getZ();
			}
			maximumCoord.pos = new BlockPos(x, y, z);
		}
	...
	private void _onAssimilated(MultiblockControllerBase otherController) {
		if (referenceCoord != null) {
			if (worldObj.getChunkProvider().chunkExists(referenceCoord.getChunkX(), referenceCoord.getChunkZ())) {
				TileEntity te = this.worldObj.getTileEntity(referenceCoord.pos);
	...
	public final void updateMultiblockEntity() {
		...
		if (worldObj.isRemote) {
			...
			if (minimumCoord != null && maximumCoord != null
					&& WorldUtils.checkChunksExist(worldObj, minimumCoord.pos.getX(), minimumCoord.pos.getY(),
							minimumCoord.pos.getZ(), maximumCoord.pos.getX(), maximumCoord.pos.getY(),
							maximumCoord.pos.getZ())) {
				int minChunkX = minimumCoord.pos.getX() >> 4;
				int minChunkZ = minimumCoord.pos.getZ() >> 4;
				int maxChunkX = maximumCoord.pos.getX() >> 4;
				int maxChunkZ = maximumCoord.pos.getZ() >> 4;
	...
	/**
	 * @param level
	 *            the level of the block on the multiblock, starting at 0 for
	 *            the bottom.
	 * @param world
	 ...
	 *             faces
	 */
	protected void isBlockGoodForExteriorLevel(int level, World world, BlockPos pos)
			throws MultiblockValidationException {
		Block block = world.getBlockState(pos).getBlock();
		throw new MultiblockValidationException(StringUtil.localizeAndFormatRaw("for.multiblock.error.invalid.interior",
				pos, block.getLocalizedName()));
	}
	...
	public void recalculateMinMaxCoords() {
		...
		for (IMultiblockPart part : connectedParts) {
			int xMin = minimumCoord.pos.getX();
			int xMax = maximumCoord.pos.getX();
			int yMin = minimumCoord.pos.getY();
			int yMax = maximumCoord.pos.getY();
			int zMin = minimumCoord.pos.getZ();
			int zMax = maximumCoord.pos.getZ();
			if (part.getPos().getX() < minimumCoord.pos.getX()) {
				xMin = part.getPos().getX();
			}
			if (part.getPos().getX() > maximumCoord.pos.getX()) {
				xMax = part.getPos().getX();
			}
			if (part.getPos().getY() < minimumCoord.pos.getY()) {
				yMin = part.getPos().getY();
			}
			if (part.getPos().getY() > maximumCoord.pos.getY()) {
				yMax = part.getPos().getY();
			}
			if (part.getPos().getZ() < minimumCoord.pos.getZ()) {
				zMin = part.getPos().getZ();
			}
			if (part.getPos().getZ() > maximumCoord.pos.getZ()) {
				zMax = part.getPos().getZ();
			}
			minimumCoord.pos = new BlockPos(xMin, yMin, zMin);
			maximumCoord.pos = new BlockPos(xMax, yMax, zMax);
		}
	}
	...
	public final CoordTriplet getCenterCoord() {
		...
		return new CoordTriplet((minCoord.pos.getX() + maxCoord.pos.getX()) / 2,
				(minCoord.pos.getY() + maxCoord.pos.getY()) / 2, (minCoord.pos.getZ() + maxCoord.pos.getZ()) / 2);
	}
	...
	public final CoordTriplet getTopCenterCoord() {
		...
		return new CoordTriplet((minCoord.pos.getX() + maxCoord.pos.getX()) / 2, maxCoord.pos.getY(),
				(minCoord.pos.getZ() + maxCoord.pos.getZ()) / 2);
	}

	public final boolean isCoordInMultiblock(int x, int y, int z) {
		return (x >= minimumCoord.pos.getX() && x <= maximumCoord.pos.getX())
				&& (y >= minimumCoord.pos.getY() && y <= maximumCoord.pos.getY())
				&& (z >= minimumCoord.pos.getZ() && z <= maximumCoord.pos.getZ());
	}
	...
	public boolean shouldConsume(MultiblockControllerBase otherController) {
		...
		if (res < 0) {
			...
			if (res < 0) {
				...
				Proxies.log.severe("My Controller (%d): size (%d), parts: %s", hashCode(), connectedParts.size(),
						getPartsListString());
				Proxies.log.severe("Other Controller (%d): size (%d), coords: %s", otherController.hashCode(),
	...
	private void auditParts() {
		...
		Proxies.log.warning("[%s] Controller found %d dead parts during an audit, %d parts remain attached",
	...
	public Set<IMultiblockPart> checkForDisconnections() {
		...
		while (!partsToCheck.isEmpty()) {
			...
			IMultiblockPart[] nearbyParts = part.getNeighboringParts(); // Chunk-safe
																		// on
																		// server,
																		// but
																		// not
	...
	protected void markReferenceCoordForUpdate() {
		...
		if (worldObj != null && rc != null) {
			worldObj.markBlockForUpdate(rc.pos);
		}
	}
	...
	protected void markReferenceCoordDirty() {
		...
		TileEntity saveTe = worldObj.getTileEntity(referenceCoord.pos);
		worldObj.markChunkDirty(referenceCoord.pos, saveTe);
	}
