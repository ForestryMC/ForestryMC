...
public abstract class MultiblockControllerBase implements ISidedInventory, IRestrictedAccessTile, IErrorLogicSource {
	...
	public void detachBlock(IMultiblockPart part, boolean chunkUnloading) {
		...
		if (!connectedParts.remove(part)) {
			Proxies.log.warning("[%s] Double-removing part (%d) @ %d, %d, %d, this is unexpected and may cause problems. If you encounter anomalies, please tear down the reactor and rebuild it.", worldObj.isRemote ? "CLIENT" : "SERVER", part.hashCode(), part.getPos().getX(), part.getPos().getY(), part.getPos().getZ());
		}
	...
	public final void updateMultiblockEntity() {
		...
		if (worldObj.isRemote) {
			...
			if (minimumCoord != null && maximumCoord != null && WorldUtils.checkChunksExist(worldObj, minimumCoord.pos.getX(), minimumCoord.pos.getY(), minimumCoord.pos.getZ(), maximumCoord.pos.getX(), maximumCoord.pos.getY(), maximumCoord.pos.getZ())) {
	...
	public final CoordTriplet getCenterCoord() {
		...
		return new CoordTriplet(
				(minCoord.pos.getX() + maxCoord.pos.getX()) / 2,
				(minCoord.pos.getY() + maxCoord.pos.getY()) / 2,
	...
	public final boolean isCoordInMultiblock(int x, int y, int z) {
		return (x >= minimumCoord.pos.getX() && x <= maximumCoord.pos.getX()) && (y >= minimumCoord.pos.getY() && y <= maximumCoord.pos.getY()) && (z >= minimumCoord.pos.getZ() && z <= maximumCoord.pos.getZ());
	}
