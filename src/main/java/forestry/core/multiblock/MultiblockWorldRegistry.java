...
public class MultiblockWorldRegistry {
	...
	private final Set<MultiblockControllerBase> controllers; // Active
																// controllers
	private final Set<MultiblockControllerBase> dirtyControllers; // Controllers
																	// whose
																	// parts
																	// lists
																	// have
																	// changed
	private final Set<MultiblockControllerBase> deadControllers; // Controllers
																	// which are
																	// empty
		...
		if (orphanedParts.size() > 0) {
			...
			if (orphansToProcess != null && orphansToProcess.size() > 0) {
				...
				for (IMultiblockPart orphan : orphansToProcess) {
					...
					if (worldObj.getTileEntity(coord.pos) != orphan) {
