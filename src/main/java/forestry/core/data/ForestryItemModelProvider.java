...
public class ForestryItemModelProvider extends ModelProvider {
	...
	protected void registerModels() {
		...
		registerModel(LepidopterologyItems.CATERPILLAR_GE, new ModelBuilder().item().layer(0, new ResourceLocation(Constants.MOD_ID, "item/caterpillar.body2")).layer(1, new ResourceLocation(Constants.MOD_ID, "item/caterpillar.body")));
		registerModel(LepidopterologyItems.SERUM_GE, new ModelBuilder().item().layer(0, new ResourceLocation(Constants.MOD_ID, "item/liquids/jar.bottle")).layer(1, new ResourceLocation(Constants.MOD_ID, "item/liquids/jar.contents")));
		...
		for (FeatureItem<ItemCrated> featureCrated : ModuleCrates.crates) {
			registerModel(featureCrated, new ModelBuilder().parent(Constants.MOD_ID + ":item/crate-filled").loader(CrateModel.Loader.LOCATION).loaderData("variant", new JsonPrimitive(featureCrated.getIdentifier())));
		}
