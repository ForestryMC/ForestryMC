...
public final class FluidHelper {
	...
	public static boolean handleRightClick(IFluidHandler tank, EnumFacing side, EntityPlayer player, boolean fill,
			boolean drain) {
		if (player == null) {
			return false;
		}
	...
	public static boolean fillContainers(IFluidHandler fluidHandler, IInventory inv, int inputSlot, int outputSlot,
		...
		if (filled != null && (output == null
				|| (output.stackSize < output.getMaxStackSize() && InvTools.isItemEqual(filled, output)))) {
	...
	public static boolean isFillableContainer(ItemStack stack, FluidStack liquid) {
		...
		for (FluidContainerData cont : FluidContainerRegistry.getRegisteredFluidContainerData()) {
			if (cont.fluid.isFluidEqual(liquid)
					&& (cont.emptyContainer != null && cont.emptyContainer.isItemEqual(empty))) {
				return true;
			}
		}
