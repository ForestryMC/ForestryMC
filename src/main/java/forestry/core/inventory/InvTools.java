...
public abstract class InvTools {
	 ...
	 *
	 * @param stack
	 *            The ItemStack to put in the inventory.
	 * @param dest
	 *            The destination IInventory.
	...
	/**
	 * Attempts to move a single item from the source inventory into a adjacent
	 * Buildcraft pipe. If the attempt fails, the source Inventory will not be
	 * modified.
	 *
	 * @param source
	 *            The source inventory
	 * @param tileCache
	 *            The tile cache of the source block.
	...
	public static boolean removeSets(IInventory inventory, int count, ItemStack[] set, EntityPlayer player,
			boolean stowContainer, boolean oreDictionary) {
		return removeSets(inventory, count, set, 0, inventory.getSizeInventory(), player, stowContainer, oreDictionary,
				false) != null;
	}

	public static boolean removeSets(IInventory inventory, int count, ItemStack[] set, int firstSlotIndex,
			int slotCount, EntityPlayer player, boolean stowContainer, boolean oreDictionary) {
		return removeSets(inventory, count, set, firstSlotIndex, slotCount, player, stowContainer, oreDictionary,
				false) != null;
	}

	public static ItemStack[] removeSets(IInventory inventory, int count, ItemStack[] set, int firstSlotIndex,
			int slotCount, EntityPlayer player, boolean stowContainer, boolean oreDictionary, boolean craftingTools) {
		...
		for (int i = 0; i < set.length; i++) {
			...
			ItemStack removedStack = removeStack(inventory, stackToRemove, firstSlotIndex, slotCount, player,
					stowContainer, false, false);
			if (removedStack == null) {
				// remove crafting equivalents next
				removedStack = removeStack(inventory, stackToRemove, firstSlotIndex, slotCount, player, stowContainer,
						oreDictionary, craftingTools);
			}
	 ...
	 */
	private static ItemStack removeStack(IInventory inventory, ItemStack stackToRemove, int firstSlotIndex,
			int slotCount, EntityPlayer player, boolean stowContainer, boolean oreDictionary, boolean craftingTools) {
