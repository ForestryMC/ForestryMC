...
public enum BlockTypeCoreTesr implements IBlockTypeTesr {
	...
	private static IMachinePropertiesTesr<? extends TileAnalyzer> createAnalyzerProperties(Supplier<FeatureTileType<? extends TileAnalyzer>> teClass, String name) {
		MachinePropertiesTesr<? extends TileAnalyzer> machineProperties = new MachinePropertiesTesr.Builder<>(teClass, name).setParticleTexture(name + ".0").setShape(VoxelShapes::fullCube).create();
	...
	private static IMachinePropertiesTesr<? extends TileEscritoire> createEscritoireProperties(Supplier<FeatureTileType<? extends TileEscritoire>> teClass, String name) {
		...
		MachinePropertiesTesr<? extends TileEscritoire> machineProperties = new MachinePropertiesTesr.Builder<>(teClass, name).setParticleTexture(name + ".0").setShape(() -> VoxelShapes.or(desk, standLB, standLF, standRB, standRF)).create();
