...
public class CommandHelpers {
	...
	/**
	 * Avoid using this function if at all possible. Commands are processed on
	 * the server, which has no localization information.
	 *
	 * StringUtil.localize() is NOT a valid alternative for
	 * sendLocalizedChatMessage(). Messages will not be localized properly if
	 * you use StringUtil.localize().
	 */
	...
	public static void printHelp(ICommandSender sender, IForestryCommand command) {
		...
		sendLocalizedChatMessage(sender, header, "for.chat.command." + commandString + ".format",
		...
		if (commandAliases.size() > 0) {
			sendLocalizedChatMessage(sender, body, "for.chat.command.aliases",
		...
		if (!command.getChildren().isEmpty()) {
			...
			for (SubCommand child : command.getChildren()) {
				sendLocalizedChatMessage(sender,
	...
	public static List<String> addStandardTabCompletionOptions(IForestryCommand command, ICommandSender sender,
			String[] incomplete, BlockPos pos) {
