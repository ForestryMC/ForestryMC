...
public final class CommandSaveStats extends SubCommand {
	...
	public void processSubCommand(ICommandSender sender, String[] args) throws PlayerNotFoundException {
		...
		statistics.add(StatCollector.translateToLocalFormatted(saveHelper.getUnlocalizedSaveStatsString(),
				player.getDisplayName(), date));
		statistics.add("");
		statistics.add(StatCollector.translateToLocalFormatted("for.chat.command.forestry.stats.save.mode",
		...
		File file = new File(Proxies.common.getForestryRoot(), "config/" + Defaults.MOD.toLowerCase(Locale.ENGLISH)
				+ "/stats/" + player.getDisplayName() + '-' + saveHelper.getFileSuffix() + ".log");
		try {
		...
		CommandHelpers.sendLocalizedChatMessage(sender, "for.chat.command.forestry.stats.save.saved",
	...
	private static String generateSpeciesListEntry(IAlleleSpecies species, IBreedingTracker tracker) {
		...
		return speciesListEntry(discovered, blacklisted, notCounted, species.getUID(), species.getName(),
	...
	private static String speciesListEntry(String discovered, String blacklisted, String notCounted, String UID,
		...
		return String.format("[ %-2s ] [ %-2s ] [ %-2s ]\t%-40s %-20s %-20s", discovered, blacklisted, notCounted, UID,
