...
public class Version {
	...
	public static final String[] FAILED_CHANGELOG = new String[] {
			String.format("Unable to retrieve changelog for %s", Defaults.MOD) };
	...
	private static class VersionChecker extends Thread {
		...
		public void run() {
			try {
				...
				if (result >= 0) {
					Proxies.log.finer("Using the latest version [" + VERSION + " (build:" + BUILD_NUMBER
				...
				Proxies.log.warning("Using outdated version [" + VERSION + " (build:" + BUILD_NUMBER
						+ ")] for Minecraft " + mcVersion + ". Consider updating.");
