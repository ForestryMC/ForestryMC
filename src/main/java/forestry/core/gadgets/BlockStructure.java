...
public abstract class BlockStructure extends BlockForestry {
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side,
		...
		// If the player's hands are empty and they right-click on a multiblock,
		// they get a
		...
		if (player.getCurrentEquippedItem() == null && !controller.isAssembled()) {
			if (controller != null) {
				...
				player.addChatMessage(
						new ChatComponentText(StatCollector.translateToLocal("for.multiblock.error.notConnected")));
				return true;
			}
		}
