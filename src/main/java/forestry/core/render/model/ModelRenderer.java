...
public class ModelRenderer implements IModelRenderer {
	...
	public void setColorOpaque_F(int r, int g, int b) {
		int alpha = 0xff;
		color = Math.min(0xff, Math.max(0, r)) << 16 | Math.min(0xff, Math.max(0, g)) << 8
	...
	public void addVertexWithUV(EnumFacing face, double x, double y, double z, double u, double v) {
		...
		if (point == 4) {
			brightness = -1;
			int[] vertData = new int[] { Float.floatToRawIntBits(points[0][0]), Float.floatToRawIntBits(points[0][1]),
					Float.floatToRawIntBits(points[0][2]), brightness, Float.floatToRawIntBits(points[0][3]),
					Float.floatToRawIntBits(points[0][4]), 0,

					Float.floatToRawIntBits(points[1][0]), Float.floatToRawIntBits(points[1][1]),
					Float.floatToRawIntBits(points[1][2]), brightness, Float.floatToRawIntBits(points[1][3]),
					Float.floatToRawIntBits(points[1][4]), 0,

					Float.floatToRawIntBits(points[2][0]), Float.floatToRawIntBits(points[2][1]),
					Float.floatToRawIntBits(points[2][2]), brightness, Float.floatToRawIntBits(points[2][3]),
					Float.floatToRawIntBits(points[2][4]), 0,

					Float.floatToRawIntBits(points[3][0]), Float.floatToRawIntBits(points[3][1]),
					Float.floatToRawIntBits(points[3][2]), brightness, Float.floatToRawIntBits(points[3][3]),
					Float.floatToRawIntBits(points[3][4]), 0, };
	...
	private void addFace(EnumFacing face, boolean isEdge, Vector3f to, Vector3f from, float[] defUVs2,
			TextureAtlasSprite texture) {

		faces.add(new RenderFace(face, isEdge, color, to, from, defUVs2, texture));
	}
	...
	public class RenderFace {
		...
		public RenderFace(EnumFacing face, boolean isEdge, int color, Vector3f to, Vector3f from, float[] defUVs2,
