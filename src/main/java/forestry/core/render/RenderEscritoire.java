...
public class RenderEscritoire extends TileEntitySpecialRenderer implements IBlockRenderer {
	...
	private static final ResourceLocation texture = new ForestryResource(
			Defaults.TEXTURE_PATH_BLOCKS + "/escritoire.png");
	...
	private void render(ItemStack itemstack, EnumFacing orientation, double x, double y, double z) {
		...
		/*
		 * if(itemstack != null) { float renderScale = 1.0f;
		 * 
		 * GL11.glPushMatrix(); GL11.glTranslatef((float) x, (float) y, (float)
		 * z); GL11.glTranslatef(0.6f, 0.8f, 0.5f); GL11.glRotatef(90.0f, 1.0F,
		 * 0.0F, 0.0F); GL11.glScalef(renderScale, renderScale, renderScale);
		 * 
