	...
	private static class RawContentModel extends ContentModel {
		...
		public ContentModel bake() {
			...
			if (bakedModel != null) {
				...
				if (!general.isEmpty()) {
					TransformationMatrix frontTransform = new TransformationMatrix(new Vector3f(-CONTENT_RENDER_OFFSET_X, 0, CONTENT_RENDER_OFFSET_Z), null, new Vector3f(0.5F, 0.5F, 1F), null);
					...
					TransformationMatrix backTransform = new TransformationMatrix(new Vector3f(-CONTENT_RENDER_OFFSET_X, 0, -CONTENT_RENDER_OFFSET_Z), null, new Vector3f(0.5F, 0.5F, 1f), TransformationHelper.quatFromXYZ(new Vector3f(0, (float) Math.PI, 0), false));
					...
					TransformationMatrix frontTransform = new TransformationMatrix(new Vector3f(-CONTENT_RENDER_OFFSET_X, 0, 0), null, new Vector3f(0.5F, 0.5F, CONTENT_RENDER_BLOCK_Z_SCALE), null);
					TRSRBakedModel frontModel = new TRSRBakedModel(guiModel, frontTransform);
