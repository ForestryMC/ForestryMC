...
public class CrateModel implements IModelGeometry<CrateModel> {
    ...
    private IBakedModel getCustomContentModel(
        ...
        IUnbakedModel model;
        if (!ResourceUtil.resourceExists(new ResourceLocation(
    ...
    public IBakedModel bake(
        ...
        if (bakedQuads.isEmpty()) {
            IBakedModel bakedModel = bakery.getBakedModel(new ModelResourceLocation(
                    Constants.MOD_ID + ":crate-filled",
                    "inventory"
            ), transform, spriteGetter);
            if (bakedModel != null) {
    ...
    public static class Loader implements IModelLoader {
        ...
        public IModelGeometry read(JsonDeserializationContext deserializationContext, JsonObject modelContents) {
            ResourceLocation registryName = new ResourceLocation(
                    Constants.MOD_ID,
                    JSONUtils.getString(modelContents, "variant")
            );
            Item item = ForgeRegistries.ITEMS.getValue(registryName);
            if (!(item instanceof ItemCrated)) {
                return ModelLoaderRegistry.getModel(new ModelResourceLocation(new ResourceLocation(
                        Constants.MOD_ID,
                        CreateItems.CRATE.getIdentifier()
                ), "inventory"), deserializationContext, modelContents);
            }
