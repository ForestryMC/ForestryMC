...
//
//		setBlockState(world, materials.fenceGate.with(HorizontalBlock.FACING, Direction.EAST), 8, 1, 8, structBoundingBox);
//		setBlockState(world, materials.fenceGate.with(HorizontalBlock.FACING, Direction.EAST), 1, 1, 8, structBoundingBox);
//		setBlockState(world, materials.fenceGate.with(HorizontalBlock.FACING, Direction.NORTH), 4, 1, 10, structBoundingBox);
//
//		// Flowers
...
//
//		// Floor
//		BlockState slabFloor = materials.slabs.with(SlabBlock.HALF, SlabBlock.EnumBlockHalf.BOTTOM);
...
//
//		// Ceiling
//		BlockState slabCeiling = materials.slabs.with(SlabBlock.HALF, SlabBlock.EnumBlockHalf.TOP);
//		fillWithBlocks(world, structBoundingBox, 1, 4, 1, 7, 4, 4, slabCeiling, slabCeiling, false);
//
//		BlockState logBracing = materials.logs.with(LogBlock.LOG_AXIS, LogBlock.EnumAxis.X);
...
//
//		BlockState glassPaneState = Blocks.GLASS_PANE.getDefaultState();
//
...
//		// stairs over window
//		BlockState eastStairs = materials.stairs.with(StairsBlock.FACING, Direction.EAST);
...
//		// stairs over window
//		BlockState westStairs = materials.stairs.with(StairsBlock.FACING, Direction.WEST);
...
//			BlockState escritoireBlock = ModuleCore.getBlocks().escritoire.getDefaultState().with(BlockBase.FACING, Direction.EAST);
//			setBlockState(world, escritoireBlock, 1, 1, 3, structBoundingBox);
//		}
//
//		BlockState airState = Blocks.AIR.getDefaultState();
//
//		this.setBlockState(world, materials.door.with(DoorBlock.FACING, Direction.NORTH), 2, 1, 0, structBoundingBox);
//		this.setBlockState(world, materials.door.with(DoorBlock.FACING, Direction.NORTH).with(DoorBlock.HALF, DoorBlock.EnumDoorHalf.UPPER), 2, 2, 0, structBoundingBox);
//
//		this.setBlockState(world, Blocks.TORCH.getDefaultState().with(TorchBlock.FACING, Direction.NORTH), 2, 3, 1, structBoundingBox);
//
...
//			setBlockState(world, materials.stairs.with(StairsBlock.FACING, Direction.NORTH), 2, 0, -1, structBoundingBox);
//		}
//
//		setBlockState(world, airState, 6, 1, 5, structBoundingBox);
//		setBlockState(world, airState, 6, 2, 5, structBoundingBox);
//
//		this.setBlockState(world, Blocks.TORCH.getDefaultState().with(TorchBlock.FACING, Direction.SOUTH), 6, 3, 4, structBoundingBox);
//
//		this.setBlockState(world, materials.door.with(DoorBlock.FACING, Direction.SOUTH), 6, 1, 5, structBoundingBox);
//		this.setBlockState(world, materials.door.with(DoorBlock.FACING, Direction.SOUTH).with(DoorBlock.HALF, DoorBlock.EnumDoorHalf.UPPER), 6, 2, 5, structBoundingBox);
//
//		for (int i = 0; i < 5; ++i) {
//			for (int j = 0; j < 9; ++j) {
...
//	}
//
//	private void buildRoof(World world, MutableBoundingBox structBoundingBox) {
//		for (int z = -1; z <= 2; ++z) {
//			for (int x = 0; x <= 8; ++x) {
//				BlockState northStairs = materials.stairs.with(StairsBlock.FACING, Direction.NORTH);
//				setBlockState(world, northStairs, x, 4 + z, z, structBoundingBox);
//
//				BlockState southStairs = materials.stairs.with(StairsBlock.FACING, Direction.SOUTH);
//				setBlockState(world, southStairs, x, 4 + z, 5 - z, structBoundingBox);
//			}
//		}
//	}
//
//	private void buildGarden(World world, MutableBoundingBox box) {
//
...
//	}
//
//	private void plantFlowerGarden(World world, MutableBoundingBox box, int minX, int minY, int minZ, int maxX, int maxY, int maxZ) {
//
//		if (structureType == 1) { // desert
...
//	}
//
//	private void buildApiaries(World world, MutableBoundingBox box) {
//		populateApiary(world, box, new BlockPos(3, 1, 8));
//		populateApiary(world, box, new BlockPos(6, 1, 8));
//	}
//
//	private void populateApiary(World world, MutableBoundingBox box, BlockPos pos) {
...
//		}
//
//		BlockState blockState = world.getBlockState(posNew);
//		if (ModuleApiculture.getBlocks().beeHouse == blockState.getBlock() || !world.isBlockLoaded(posNew.down())) {
//			return;
//		}
//
//		BlockState beeHouseDefaultState = ModuleApiculture.getBlocks().beeHouse.getDefaultState();
...
//	}
//
//	private boolean isAirBlockAtCurrentPosition(World world, BlockPos pos, MutableBoundingBox box) {
//		BlockState blockStateFromPos = getBlockStateFromPos(world, pos.getX(), pos.getY(), pos.getZ(), box);
//		return blockStateFromPos.getBlock().isAir(blockStateFromPos, world, pos);
//	}
//
//	private static class Materials {
//		public final BlockState planks;
//		public final BlockState slabs;
//		public final BlockState logs;
//		public final BlockState stairs;
//		public final BlockState fence;
...
//			this.logs = woodAccess.getBlock(woodType, WoodBlockKind.LOG, fireproof).with(LogBlock.LOG_AXIS, LogBlock.EnumAxis.X);
