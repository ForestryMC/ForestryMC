...
public enum HiveDescription implements IHiveDescription {
	...
	FOREST(1, 3.0f, BeeDefinition.FOREST, HiveManager.genHelper.tree()), MEADOWS(2, 1.0f, BeeDefinition.MEADOWS,
			HiveManager.genHelper.ground(Blocks.dirt, Blocks.grass)), DESERT(3, 1.0f, BeeDefinition.MODEST,
					HiveManager.genHelper.ground(Blocks.dirt, Blocks.grass, Blocks.sand, Blocks.sandstone)), JUNGLE(4,
							4.0f, BeeDefinition.TROPICAL, HiveManager.genHelper.tree()), END(5, 4.0f,
									BeeDefinition.ENDED, HiveManager.genHelper.ground(Blocks.end_stone)) {
								@Override
		...
		public void postGen(World world, BlockPos pos) {
			if (world.isAirBlock(new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ()))) {
				world.setBlockState(new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ()),
						Blocks.snow_layer.getStateFromMeta(0), 0);
			}
		}
	},
