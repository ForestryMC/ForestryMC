...
public class BeeItemRenderer implements IItemRenderer {
	 ...
	 * 
	 * IIcon background =
	 * TextureManager.getInstance().getDefault("habitats/desert"); float xPos =
	 * 1.0f; float yPos = 1.0f; float width = 1f; float height = 1f;
	 * 
	 * Proxies.common.bindTexture(SpriteSheet.ITEMS); Tessellator tessellator =
	 * Tessellator.instance; tessellator.startDrawingQuads();
	 * tessellator.addVertexWithUV((double)(xPos + 0), (double)(yPos + height),
	 * 0f, (double)background.getMinU(), (double)background.getMaxV());
	 * tessellator.addVertexWithUV((double)(xPos + width), (double)(yPos +
	 * height), 0f, (double)background.getMaxU(), (double)background.getMaxV());
	 * tessellator.addVertexWithUV((double)(xPos + width), (double)(yPos + 0),
	 * 0f, (double)background.getMaxU(), (double)background.getMinV());
	 * tessellator.addVertexWithUV((double)(xPos + 0), (double)(yPos + 0), 0f,
	 * (double)background.getMinU(), (double)background.getMinV());
	 * tessellator.draw(); }
	 */
	...
	private void renderBeeInInventory(IBee bee) {
		...
		/*
		 * GL11.glPushMatrix(); //GL11.glTranslatef(-0.3f, -2.5f, 0f);
		 * GL11.glScalef(-1.0f, 1.0f, 1.0f); //GL11.glScalef((float)Math.PI / 2,
		 * 1.0f, 1.0f); GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
		 * GL11.glRotatef(-135.0F, 0.0F, 1.0F, 0.0F); //GL11.glRotatef(-((float)
		 * Math.atan((double) (1 / 40.0F))) * 20.0F, 1.0F, 0.0F, 0.0F);
		 * renderBeeHalo(); GL11.glPopMatrix();
		 */
