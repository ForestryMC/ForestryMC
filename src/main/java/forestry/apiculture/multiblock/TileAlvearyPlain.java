...
public class TileAlvearyPlain extends TileAlveary
		implements IBeeHousing, IClimatised, IHintSource, IStreamableGui, ITitled, ITriggerProvider {
	...
	public void onMachineAssembled(MultiblockControllerBase controller) {
		...
		if (!worldObj.isRemote) {
			...
			if (getPartPosition() == PartPosition.Frame) {
				if (Utils.isWoodSlabBlock(
						worldObj.getBlockState(new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ())).getBlock())) {
					this.worldObj.setBlockState(new BlockPos(pos.getX(), pos.getY(), pos.getZ()),
							ForestryBlock.alveary.block().getStateFromMeta(TileAlveary.ENTRANCE_META), 2);
				}
			}
		}
	}
	...
	public void onMachineBroken() {
		...
		if (!worldObj.isRemote) {
			...
			this.worldObj.setBlockState(pos, ForestryBlock.alveary.block().getStateFromMeta(TileAlveary.PLAIN_META), 2);
		}
	}
	...
	public void openGui(EntityPlayer player) {
		if (isConnected()) {
			player.openGui(ForestryAPI.instance, GuiId.AlvearyGUI.ordinal(), worldObj, pos.getX(), pos.getY(),
					pos.getZ());
		}
	}
