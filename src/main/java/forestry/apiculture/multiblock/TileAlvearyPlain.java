	...
	public void onMachineAssembled(MultiblockControllerBase controller) {
		...
		if (!worldObj.isRemote) {
			...
			if (getPartPosition() == PartPosition.Frame) {
				if (Utils.isWoodSlabBlock(worldObj.getBlockState(new BlockPos(pos.getX(), pos.getY() + 1, pos.getZ())).getBlock())) {
					this.worldObj.setBlockState(new BlockPos(pos.getX(), pos.getY(), pos.getZ()), ForestryBlock.alveary.block().getStateFromMeta(TileAlveary.ENTRANCE_META), 2);
				}
			}
		}
	}
