...
public class DefaultBeeIconProvider implements IBeeIconProvider {
	...
	public void registerIcons(IIconRegister register) {
		...
		IIcon body1 = TextureManager.getSprite(register, beeIconDir + "body1");
		...
		for (int i = 0; i < EnumBeeType.values().length; i++) {
			...
			icons[i][0] = TextureManager.getSprite(register, beeTypeNameBase + ".outline");
			if (beeType == EnumBeeType.LARVAE) {
				icons[i][1] = TextureManager.getSprite(register, beeTypeNameBase + ".body");
			} else {
				icons[i][1] = body1;
			}
			icons[i][2] = TextureManager.getSprite(register, beeTypeNameBase + ".body2");
		}
	}
