...
public class AlleleBeeSpecies extends AlleleSpecies implements IAlleleBeeSpeciesCustom {
	...
	// DefaultBeeIconProvider.instance;
	...
	/*
	 * @Override public IAlleleBeeSpeciesCustom
	 * setCustomBeeIconProvider(IBeeIconProvider beeIconProvider) {
	 * this.beeIconProvider = beeIconProvider; return this; }
	 */
	 ...
	 * @Override
	 * 
	 * @SideOnly(Side.CLIENT) public ISpriteProvider getIconProvider() { return
	 * new BeeIconProviderWrapper(beeIconProvider); }
	 */
	 ...
	 * @SideOnly(Side.CLIENT)
	 * 
	 * @Override public ISprite getIcon(EnumBeeType type, int renderPass) {
	 ...
	 * 
	 * public static final DefaultBeeIconProvider instance = new
	 * DefaultBeeIconProvider();
	 * 
	 * private DefaultBeeIconProvider() {
	 * 
	 * }
	 * 
	 * private static final IIcon[][] icons = new
	 * IIcon[EnumBeeType.values().length][3];
	 * 
	 * @Override
	 * 
	 * @SideOnly(Side.CLIENT) public void registerIcons(IIconRegister register)
	 * { String beeIconDir = "bees/default/"; TextureManager textureManager =
	 * TextureManager.getInstance(); ISprite body1 =
	 * textureManager.registerTex(register, beeIconDir + "body1");
	 * 
	 * for (int i = 0; i < EnumBeeType.values().length; i++) { EnumBeeType
	 * beeType = EnumBeeType.values()[i]; if (beeType == EnumBeeType.NONE) {
	 * continue; }
	 * 
	 * String beeTypeNameBase = beeIconDir +
	 * beeType.toString().toLowerCase(Locale.ENGLISH);
	 * 
	 * icons[i][0] = textureManager.registerTex(register, beeTypeNameBase +
	 * ".outline"); if (beeType == EnumBeeType.LARVAE) { icons[i][1] =
	 * textureManager.registerTex(register, beeTypeNameBase + ".body"); } else {
	 * icons[i][1] = body1; } icons[i][2] = textureManager.registerTex(register,
	 * beeTypeNameBase + ".body2"); } }
	 * 
	 * @Override
	 * 
	 * @SideOnly(Side.CLIENT) public IIcon getIcon(EnumBeeType type, int
	 * renderPass) { return icons[type.ordinal()][renderPass]; } }
	 * 
	 ...
	 * 
	 * @Override public IIcon getIcon(short texUID) { return null; }
	 * 
