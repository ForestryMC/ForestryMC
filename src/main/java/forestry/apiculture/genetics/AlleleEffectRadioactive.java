...
public class AlleleEffectRadioactive extends AlleleEffectThrottled {
	...
	public IEffectData doEffectThrottled(IBeeGenome genome, IEffectData storedData, IBeeHousing housing) {
		...
		AxisAlignedBB hurtBox = AxisAlignedBB.fromBounds(min.getX(), min.getY(), min.getZ(), max.getX(), max.getY(),
				max.getZ());
		...
		for (int i = 0; i < 20; i++) {
			...
			if (posBlock.getX() == posHousing.getX() && posBlock.getY() == posHousing.getZ()
					&& posBlock.getZ() <= posHousing.getZ()) {
				continue;
			}
