...
public class BeekeepingLogic implements IBeekeepingLogic, IStreamable {
	...
	private IBee queen;
	private ItemStack queenStack; // used to detect server changes and sync
									// clientQueen
	...
	private void tickBreed() {
		...
		if (!ForestryItem.beeDroneGE.isItemEqual(droneStack)
				|| !ForestryItem.beePrincessGE.isItemEqual(princessStack)) {
			beeProgress = 0;
			return;
		}
