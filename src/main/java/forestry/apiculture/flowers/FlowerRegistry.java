...
public final class FlowerRegistry implements IFlowerRegistry {
	...
	private final Set<String> defaultFlowerTypes = ImmutableSet.of(FlowerManager.FlowerTypeVanilla,
			FlowerManager.FlowerTypeNether, FlowerManager.FlowerTypeCacti, FlowerManager.FlowerTypeMushrooms,
			FlowerManager.FlowerTypeEnd, FlowerManager.FlowerTypeJungle, FlowerManager.FlowerTypeSnow,
			FlowerManager.FlowerTypeWheat, FlowerManager.FlowerTypeGourd);

	private final HashMultimap<String, Block> registeredBlocks; // quick first
																// check
	...
	public BlockPos getAcceptedFlowerCoordinates(IBeeHousing beeHousing, IBee bee, String flowerType) {
		...
		Vect housingPos = new Vect(beeHousing.getCoordinates()).add(-area.getX() / 2, -area.getY() / 2,
				-area.getZ() / 2);
	...
	private static boolean isAcceptedFlower(String flowerType, Set<Block> acceptedBlocks, Set<IFlower> acceptedFlowers,
		...
		if (PluginManager.Module.AGRICRAFT.isEnabled() && (flowerType.equals(FlowerManager.FlowerTypeWheat)
	...
	private void registerVanillaGrowthRules() {
		registerGrowthRule(new VanillaDirtGrassGrowthRule(), FlowerManager.FlowerTypeVanilla,
				FlowerManager.FlowerTypeSnow);
		registerGrowthRule(new VanillaSnowGrowthRule(), FlowerManager.FlowerTypeSnow);
		registerGrowthRule(new VanillaFlowerPotGrowthRule(), FlowerManager.FlowerTypeVanilla,
				FlowerManager.FlowerTypeSnow, FlowerManager.FlowerTypeMushrooms, FlowerManager.FlowerTypeCacti,
		...
		registerGrowthRule(new VanillaFertilizeGrowthRule(Blocks.melon_stem, Blocks.pumpkin_stem),
