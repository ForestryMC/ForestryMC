...
public class GuiBeealyzer extends GuiAlyzer {
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int mouseX, int mouseY) {
		...
		for (int k = AlyzerInventory.SLOT_SPECIMEN; k <= AlyzerInventory.SLOT_ANALYZE_5; k++) {
	...
	private void drawAnalyticsPage1(IBee bee, EnumBeeType type) {
...

		{
			...
			drawSpeciesRow(StringUtil.localize("gui.species"), bee, EnumBeeChromosome.SPECIES,
					checkCustomName(customPrimaryBeeKey), checkCustomName(customSecondaryBeeKey));
			newLine();
		}
		...
		drawRow(StringUtil.localize("gui.area"), new Vect(activeTerritory.getValue()).toString(),
				new Vect(inactiveTerritory.getValue()).toString(), bee, EnumBeeChromosome.TERRITORY);
	...
	private void drawAnalyticsPage2(IBee bee, EnumBeeType type) {
		...
		drawRow(StringUtil.localize("gui.climate"),
				...
				AlleleManager.climateHelper.toDisplay(bee.getGenome().getSecondary().getTemperature()), bee,
		...
		drawRow(StringUtil.localize("gui.humidity"),
				...
				AlleleManager.climateHelper.toDisplay(bee.getGenome().getSecondary().getHumidity()), bee,
				EnumBeeChromosome.SPECIES);
		...
		String secondary = StringUtil.readableBoolean(
				((AlleleBoolean) bee.getGenome().getInactiveAllele(EnumBeeChromosome.TOLERANT_FLYER)).getValue(), yes,
				no);
		...
		secondary = StringUtil.readableBoolean(
				((AlleleBoolean) bee.getGenome().getInactiveAllele(EnumBeeChromosome.CAVE_DWELLING)).getValue(), yes,
				no);
