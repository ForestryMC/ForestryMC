...
public class BlockAlveary extends BlockStructure {
	...
	public enum AlvearyTypes implements IStringSerializable {
		ALVEARY_PLAIN, ALVEARY_ENTRANCE, ALVEARY_SWARMER, ALVEARY_HEATER, ALVEARY_FAN, ALVEARY_HYGRO, ALVEARY_STABILISER, ALVEARY_SIEVE;
	...
	public BlockAlveary() {
		...
		setDefaultState(this.blockState.getBaseState().withProperty(ALVEARYTYPE, AlvearyTypes.ALVEARY_PLAIN)
				.withProperty(STATE, State.OFF).withProperty(TYPE, Type.MIDDLE));
	}
	...
	public static class AlvearyStateMapper extends StateMapperBase {
		...
		protected ModelResourceLocation getModelResourceLocation(IBlockState state) {
			...
			if (linkedhashmap.get(ALVEARYTYPE) == AlvearyTypes.ALVEARY_SIEVE
					|| linkedhashmap.get(ALVEARYTYPE) == AlvearyTypes.ALVEARY_ENTRANCE
					|| linkedhashmap.get(ALVEARYTYPE) == AlvearyTypes.ALVEARY_STABILISER
					|| linkedhashmap.get(ALVEARYTYPE) == AlvearyTypes.ALVEARY_HYGRO
					|| linkedhashmap.get(ALVEARYTYPE) == AlvearyTypes.ALVEARY_PLAIN) {
				linkedhashmap.remove(STATE);
			}
			String s = String.format("%s:%s",
					((ResourceLocation) Block.blockRegistry.getNameForObject(state.getBlock())).getResourceDomain(),
					"apiculture/" + ALVEARYTYPE.getName((Comparable) linkedhashmap.remove(ALVEARYTYPE)));
			;
