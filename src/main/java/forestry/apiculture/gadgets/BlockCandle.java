...
public class BlockCandle extends BlockTorch implements IModelRegister {
	...
	static {
		...
		lightingItems = new HashSet<Item>(
	...
	public BlockCandle() {
		...
		setDefaultState(
				this.blockState.getBaseState().withProperty(STATE, State.OFF).withProperty(FACING, EnumFacing.UP));
	}
	...
	public void registerModel(Item item, IModelManager manager) {
		manager.registerItemModel(item, 0, ".off");
		manager.registerItemModel(item, 1, ".on");
	};
	...
	public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumFacing side,
		...
		if (!isLit) {
			...
			} else if (StackUtils.equals(this, heldItem) && isLit(heldItem)) {
				toggleLitState = true;
			}
		}

		if (heldItem != null) {
			if (StackUtils.equals(this, heldItem)) {
				if (!isLit(heldItem)) {
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		if (dropStack == null) {
			dropStack = getCandleDrop((World) world, pos);
		}
	...
	private ItemStack getCandleDrop(World world, BlockPos pos) {
		TileEntity tileEntity = world.getTileEntity(pos);
		if (!(tileEntity instanceof TileCandle)) {
			return null;
		}
		...
		if (colour != 0xffffff) {
			// When dropped, tag new item stack with colour. Unless it's white,
			// then do no such thing for maximum stacking.
