...
public class FarmableStacked implements IFarmable {
	...
	public ICrop getCropAt(World world, BlockPos pos) {
		if (world.getBlockState(new BlockPos(pos.getX(), pos.getY() + (matureHeight - 1), pos.getZ()))
				.getBlock() != block) {
			return null;
		}
		return new CropBlock(world, block, matureMeta,
				new Vect(pos.getX(), pos.getY() + (matureHeight - 1), pos.getZ()));
	}
