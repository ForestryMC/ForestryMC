...
public class CropBasicAgriCraft extends Crop {
	...
	protected Collection<ItemStack> harvestBlock(Vect pos) {
		...
		if (harvest.size() > 1) {
			harvest.remove(1); // AgriCraft returns cropsticks in 0, seeds in 1
								// in getDrops, removing since harvesting
								// doesn't return them.
		}
		harvest.remove(0);
	...
	public String toString() {
		return String.format("CropBasicAgriCraft [ position: [ %s ]; block: %s; meta: %s ]", position.toString(),
