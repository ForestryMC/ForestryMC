...
public class GuiFarm extends GuiForestry<ContainerFarm, TileFarm> {
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int mouseX, int mouseY) {
		...
		if (fertilizerRemain > 0) {
			drawTexturedModalRect(guiLeft + 81, guiTop + 94 + 17 - fertilizerRemain, xSize, 17 - fertilizerRemain, 4,
					fertilizerRemain);
		}
	}
	...
	private class FarmLogicSlot extends Widget {
		...
		public void draw(int startX, int startY) {
			...
			if (getIconIndex() != null) {
				...
				manager.gui.drawTexturedModelRect(startX + xPos, startY + yPos, getIconIndex(), 16, 16);
		...
		protected final ToolTip toolTip = new ToolTip(250) {
			...
			public void refresh() {
				...
				toolTip.add("Water: " + getLogic().getWaterConsumption(
						inventory.getFarmController().getFarmLedgerDelegate().getHydrationModifier()));
			}
		};
	}
