...
public enum FarmDefinition implements IStringSerializable {
	CROPS("crops", EnumElectronTube.BRONZE, FarmLogicCrops::new) {
		...
		protected void initProperties(IFarmPropertiesBuilder properties) {
			wateredProperties(properties).addSoil(Blocks.DIRT).addFarmables("Wheat").setIcon(() -> new ItemStack(Items.WHEAT));
		}
	}, GOURD("gourd", EnumElectronTube.LAPIS, FarmLogicGourd::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.setIcon(() -> new ItemStack(Items.MELON)).addSoil(Blocks.DIRT).setFertilizer(10).setWater(hydrationModifier -> (int) (40 * hydrationModifier));
		}
	}, SHROOM("shroom", EnumElectronTube.APATITE, FarmLogicMushroom::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.addSoil(Blocks.MYCELIUM).addSoil(Blocks.PODZOL).setWater(hydrationModifier -> (int) (80 * hydrationModifier)).setFertilizer(20).setIcon(() -> new ItemStack(Blocks.RED_MUSHROOM));
		}
	}, INFERNAL("infernal", EnumElectronTube.BLAZE, FarmLogicInfernal::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.addSoil(Blocks.SOUL_SAND).addProducts(new ItemStack(Items.NETHER_WART)).setWater(0).setFertilizer(20).setIcon(() -> new ItemStack(Items.NETHER_WART));
		}
	}, POALES("poales", EnumElectronTube.TIN, FarmLogicReeds::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.addSoil(Blocks.SAND).addSoil(Blocks.DIRT).setFertilizer(10).setWater(hydrationModifier -> (int) (20 * hydrationModifier)).setIcon(() -> new ItemStack(Items.SUGAR_CANE));
		}
	}, SUCCULENTES("succulentes", EnumElectronTube.COPPER, FarmLogicSucculent::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.addSoil(Blocks.SAND).setFertilizer(10).setWater(1).setIcon(() -> new ItemStack(Items.GREEN_DYE));
		}
	}, ENDER("ender", EnumElectronTube.ENDER, FarmLogicEnder::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.addSoil(Blocks.END_STONE).setIcon(() -> new ItemStack(Items.ENDER_EYE)).setFertilizer(20).setWater(0);
		}
	}, ARBOREAL("arboreal", EnumElectronTube.GOLD, FarmLogicArboreal::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.addSoil(new ItemStack(Blocks.DIRT), CoreBlocks.HUMUS.defaultState()).addSoil(CoreBlocks.HUMUS.stack(), CoreBlocks.HUMUS.defaultState()).addProducts(new ItemStack(Blocks.SAND)).setFertilizer(10).setWater(hydrationModifier -> (int) (10 * hydrationModifier)).setIcon(() -> new ItemStack(Blocks.OAK_SAPLING));
		}
	}, PEAT("peat", EnumElectronTube.OBSIDIAN, FarmLogicPeat::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			wateredProperties(properties).addSoil(CoreBlocks.BOG_EARTH.stack(), CoreBlocks.BOG_EARTH.defaultState()).addProducts(CoreItems.PEAT.stack(), new ItemStack(Blocks.DIRT)).setIcon(CoreItems.PEAT::stack).setFertilizer(2);
		}
	}, ORCHARD("orchard", EnumElectronTube.EMERALD, FarmLogicOrchard::new) {
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.setFertilizer(10).setWater(hydrationModifier -> (int) (40 * hydrationModifier)).setIcon(() -> CoreItems.FRUITS.stack(ItemFruit.EnumFruit.CHERRY));
				...
				if (treeRoot != null) {
					for (ITree tree : treeRoot.getIndividualTemplates()) {
						...
						if (fruitProvider != AlleleFruits.fruitNone.getProvider()) {
							properties.addSeedlings(treeRoot.getTypes().createStack(tree, EnumGermlingType.SAPLING)).addProducts(fruitProvider.getProducts().getPossibleStacks()).addProducts(fruitProvider.getSpecialty().getPossibleStacks());
						}
					}
				}
			}
		...
		@Override
		protected void initProperties(IFarmPropertiesBuilder properties) {
			properties.addSoil(Blocks.JUNGLE_LOG).addSeedlings(new ItemStack(Items.COCOA_BEANS)).addProducts(new ItemStack(Items.COCOA_BEANS)).setFertilizer(120).setWater(hydrationModifier -> (int) (20 * hydrationModifier)).setIcon(() -> new ItemStack(Items.COCOA_BEANS));
		}
	}/*,
	//TODO: Mod combat
	...
	FarmDefinition(String identifier, EnumElectronTube tube, BiFunction<IFarmProperties, Boolean, IFarmLogic> factory, String module) {
		...
		IFarmPropertiesBuilder builder = FarmRegistry.getInstance().getPropertiesBuilder("farm" + camelCase).setFactory(factory).setTranslationKey("for.farm." + identifier).addFarmables("farm" + camelCase);
	...
	protected IFarmPropertiesBuilder wateredProperties(IFarmPropertiesBuilder builder) {
		return builder.setWater((hydrationModifier) -> (int) (20 * hydrationModifier)).setFertilizer(5);
	}
