...
public class FarmController extends RectangularMultiblockControllerBase implements IFarmController {
	...
	private final Map<FarmDirection, List<FarmTarget>> targets = new EnumMap<FarmDirection, List<FarmTarget>>(
			FarmDirection.class);
	private int allowedExtent = 0;

	private IFarmLogic harvestProvider; // The farm logic which supplied the
										// pending crops.
	...
	// active components are stored with a tick offset so they do not all tick
	// together
	...
	private final Map<FarmDirection, IFarmLogic> farmLogics = new EnumMap<FarmDirection, IFarmLogic>(
			FarmDirection.class);
	...
	public EnumTemperature getTemperature() {
		CoordTriplet coords = getReferenceCoord();
		return EnumTemperature.getFromBiome(getBiome(), coords.pos);
	}
	...
	public float getExactTemperature() {
		CoordTriplet coords = getReferenceCoord();
		return getBiome().getFloatTemperature(coords.pos);
	}
	...
	public int[] getCoords() {
		if (coords == null) {
			...
			coords = new int[] { centerCoord.pos.getX(), centerCoord.pos.getY(), centerCoord.pos.getZ() };
		}
		return coords;
	}
	...
	private void setUpFarmlandTargets() {
		...
		int sizeNorthSouth = Math.abs(max.pos.getZ() - min.pos.getZ()) + 1;
		int sizeEastWest = Math.abs(max.pos.getX() - min.pos.getX()) + 1;
	...
	private static void createTargets(World world, Map<FarmDirection, List<FarmTarget>> targets, Vect targetStart,
		...
		for (FarmDirection farmSide : FarmDirection.values()) {
			...
			// targets extend sideways in a pinwheel pattern around the farm, so
			// they need to go a little extra distance
	...
	private static boolean cultivateTarget(FarmTarget target, IFarmLogic logic, Iterable<IFarmListener> farmListeners) {
		...
		if (logic.cultivate(null, target.getDirection(), target.getExtent())) {
			for (IFarmListener listener : farmListeners) {
				listener.hasCultivated(logic,
						new BlockPos(targetPosition.getX(), targetPosition.getY(), targetPosition.getZ()),
						target.getDirection(), target.getExtent());
			}
			return true;
		}
	...
	private static Collection<ICrop> harvestTarget(FarmTarget target, IFarmLogic logic,
			Iterable<IFarmListener> farmListeners) {
		Collection<ICrop> harvested = logic.harvest(new BlockPos(target.getStart().getX(),
				target.getStart().getY() + target.getYOffset(), target.getStart().getZ()), target.getDirection(),
				target.getExtent());
		...
		for (IFarmListener listener : farmListeners) {
			listener.hasScheduledHarvest(
					harvested, logic, new BlockPos(target.getStart().getX(),
							target.getStart().getY() + target.getYOffset(), target.getStart().getZ()),
					target.getDirection(), target.getExtent());
		}
