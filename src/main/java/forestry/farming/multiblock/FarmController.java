...
public class FarmController extends RectangularMultiblockControllerBase implements IFarmController {
	...
	private static boolean cultivateTarget(FarmTarget target, IFarmLogic logic, Iterable<IFarmListener> farmListeners) {
		...
		if (logic.cultivate(null, target.getDirection(), target.getExtent())) {
			for (IFarmListener listener : farmListeners) {
				listener.hasCultivated(logic, new BlockPos(targetPosition.getX(), targetPosition.getY(), targetPosition.getZ()), target.getDirection(), target.getExtent());
			}
			return true;
		}
	...
	private static Collection<ICrop> harvestTarget(FarmTarget target, IFarmLogic logic, Iterable<IFarmListener> farmListeners) {
		Collection<ICrop> harvested = logic.harvest(new BlockPos(target.getStart().getX(), target.getStart().getY() + target.getYOffset(), target.getStart().getZ()), target.getDirection(), target.getExtent());
		...
		for (IFarmListener listener : farmListeners) {
			listener.hasScheduledHarvest(harvested, logic, new BlockPos(target.getStart().getX(), target.getStart().getY() + target.getYOffset(), target.getStart().getZ()), target.getDirection(), target.getExtent());
		}
