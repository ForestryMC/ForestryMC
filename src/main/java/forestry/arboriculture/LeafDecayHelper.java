...
/**
 * Based on vanilla leaf decay, but accepts leaves that are connected further
 * from a trunk
 */
public abstract class LeafDecayHelper {
	...
	public static void leafDecay(BlockLeavesBase leaves, World world, int x, int y, int z) {
		if (world.isRemote) {
			return;
		}
		...
		if ((leafMeta & 8) != 0 && (leafMeta & 4) == 0) {
			...
			if (world.isAreaLoaded(new BlockPos(x - radius1, y - radius1, z - radius1),
					new BlockPos(x + radius1, y + radius1, z + radius1), false)) {
				for (int xOffset = -radius; xOffset <= radius; ++xOffset) {
					for (int yOffset = -radius; yOffset <= radius; ++yOffset) {
						for (int zOffset = -radius; zOffset <= radius; ++zOffset) {
							Block block = world.getBlockState(new BlockPos(x + xOffset, y + yOffset, z + zOffset))
									.getBlock();
							...
							if (!block.canSustainLeaves(world, new BlockPos(x + xOffset, y + yOffset, z + zOffset))) {
								if (block.isLeaves(world, new BlockPos(x + xOffset, y + yOffset, z + zOffset))) {
									leafDecayValues[(xOffset + arrayOffset) * xArrayMult
											+ (yOffset + arrayOffset) * yArrayMult + zOffset + arrayOffset] = IS_LEAVES;
								} else {
									leafDecayValues[(xOffset + arrayOffset) * xArrayMult
											+ (yOffset + arrayOffset) * yArrayMult + zOffset
						...
						for (int yOffset = -radius; yOffset <= radius; ++yOffset) {
							for (int zOffset = -radius; zOffset <= radius; ++zOffset) {
								if (leafDecayValues[(xOffset + arrayOffset) * xArrayMult
									...
									if (leafDecayValues[(xOffset + arrayOffset) * xArrayMult
											+ (yOffset + arrayOffset - 1) * yArrayMult + zOffset
											+ arrayOffset] == IS_LEAVES) {
										leafDecayValues[(xOffset + arrayOffset) * xArrayMult
												+ (yOffset + arrayOffset - 1) * yArrayMult + zOffset
												+ arrayOffset] = sustainedValue;
									...
									if (leafDecayValues[(xOffset + arrayOffset) * xArrayMult
											+ (yOffset + arrayOffset) * yArrayMult + zOffset + arrayOffset
											+ 1] == IS_LEAVES) {
										leafDecayValues[(xOffset + arrayOffset) * xArrayMult
												+ (yOffset + arrayOffset) * yArrayMult + zOffset + arrayOffset
												+ 1] = sustainedValue;
			...
			if (sustainValue >= 0) {
				...
				world.setBlockState(pos, stateNew.getBlock().getStateFromMeta(leafMeta & -9), 4); // stop
																									// trying
																									// to
																									// decay
			} else {
