...
public class ItemTreealyzer extends ItemInventoried {
	...
	public static class TreealyzerInventory extends AlyzerInventory implements IHintSource {
		...
		public void onSlotClick(EntityPlayer player) {
			...
			if (!tree.isAnalyzed()) {
				...
				tree.analyze();
				if (player != null) {
					TreeManager.treeRoot.getBreedingTracker(player.worldObj, player.getGameProfile())
							.registerSpecies(tree.getGenome().getPrimary());
					TreeManager.treeRoot.getBreedingTracker(player.worldObj, player.getGameProfile())
							.registerSpecies(tree.getGenome().getSecondary());
				}
	...
	public ItemStack onItemRightClick(ItemStack itemstack, World world, EntityPlayer entityplayer) {
		if (Proxies.common.isSimulating(world)) {
			entityplayer.openGui(ForestryAPI.instance, GuiId.TreealyzerGUI.ordinal(), world, (int) entityplayer.posX,
