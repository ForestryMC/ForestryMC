...
public class BlockLog extends BlockWood {
	...
	public BlockLog(boolean fireproof) {
		...
		setDefaultState(this.blockState.getBaseState().withProperty(AXIS, Axis.NORMAL).withProperty(WoodType.WOODTYPE,
				WoodType.LARCH));
	}
	...
	public void breakBlock(World world, BlockPos pos, IBlockState state) {
		...
		if (world.isAreaLoaded(new BlockPos(pos.getX() - boundary, pos.getY() - boundary, pos.getZ() - boundary),
				new BlockPos(pos.getX() + boundary, pos.getY() + boundary, pos.getZ() + boundary))) {
			for (int i = -radius; i <= radius; ++i) {
				for (int j = -radius; j <= radius; ++j) {
					for (int k = -radius; k <= radius; ++k) {
						Block neighbor = world
								.getBlockState(new BlockPos(pos.getX() + i, pos.getY() + j, pos.getZ() + k)).getBlock();
