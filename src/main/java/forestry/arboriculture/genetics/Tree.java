...
public class Tree extends Individual implements ITree, IPlantable {
	...
	public void addTooltip(List<ITextComponent> list) {
		...
		// No info 4 u!
		if (!isAnalyzed) {
			list.add(new StringTextComponent("<").append(new TranslationTextComponent("for.gui.unknown")).appendString(">"));
			return;
		}
		...
		if (!isPureBred(TreeChromosomes.SPECIES)) {
			list.add(new TranslationTextComponent("for.trees.hybrid", primary.getDisplayName(), secondary.getDisplayName()).mergeStyle(TextFormatting.BLUE));
		}

		ITextComponent sappiness = new StringTextComponent("S: ").append(genome.getActiveAllele(TreeChromosomes.SAPPINESS).getDisplayName()).mergeStyle(TextFormatting.GOLD);
		ITextComponent maturation = new StringTextComponent("M: ").append(genome.getActiveAllele(TreeChromosomes.MATURATION).getDisplayName()).mergeStyle(TextFormatting.RED);
		ITextComponent height = new StringTextComponent("H: ").append(genome.getActiveAllele(TreeChromosomes.HEIGHT).getDisplayName()).mergeStyle(TextFormatting.LIGHT_PURPLE);
		ITextComponent girth = new TranslationTextComponent("G: %1$sx%2$s", genome.getActiveAllele(TreeChromosomes.GIRTH), genome.getActiveAllele(TreeChromosomes.GIRTH)).mergeStyle(TextFormatting.AQUA);
		ITextComponent saplings = new StringTextComponent("S: ").append(genome.getActiveAllele(TreeChromosomes.FERTILITY).getDisplayName()).mergeStyle(TextFormatting.YELLOW);
		ITextComponent yield = new StringTextComponent("Y: ").append(genome.getActiveAllele(TreeChromosomes.YIELD).getDisplayName()).mergeStyle(TextFormatting.WHITE);
		...
		if (primaryFireproof) {
			list.add(new TranslationTextComponent("for.gui.fireresist").mergeStyle(TextFormatting.RED));
		}

		IAllele fruit = getGenome().getActiveAllele(TreeChromosomes.FRUITS);
		if (fruit != AlleleFruits.fruitNone) {
			IFormattableTextComponent fruitText = new StringTextComponent( "F: ").mergeStyle(TextFormatting.GREEN);

			if (!canBearFruit()) {
				fruitText.mergeStyle(TextFormatting.STRIKETHROUGH);
			}

			list.add(fruitText.append(genome.getActiveAllele(TreeChromosomes.FRUITS).getProvider().getDescription()));
		}
	}
