...
public class TreeDatabaseTab implements IDatabaseTab<ITree> {
	...
	public void createElements(IDatabaseElement container, ITree tree, ItemStack itemStack) {
		...
		container.addLine(new TranslationTextComponent("for.gui.girth"), (IAlleleValue<Integer> girth, Boolean active) -> new StringTextComponent(String.format("%sx%s", girth.getValue(), girth.getValue())), TreeChromosomes.GIRTH);
		...
		container.addLine(new TranslationTextComponent("for.gui.native"), new TranslationTextComponent("for.gui." + primarySpecies.getPlantType().getName().toLowerCase(Locale.ENGLISH)), species.isDominant());
