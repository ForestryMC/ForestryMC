...
public enum TreeDefinition implements ITreeDefinition, ITreeGenerator {
	...
	TreeDefinition(TreeBranchDefinition branch, String speciesName, String binomial, boolean dominant, EnumLeafType leafType, Color primary, Color secondary, int vanillaMeta, ItemStack vanillaWood) {
		...
		ILeafSpriteProvider leafIconProvider = TreeManager.treeFactory.getLeafSpriteProvider(leafType, primary, secondary);
		IGermlingModelProvider germlingModelProvider = new ModelProviderGermlingVanilla(vanillaMeta);

		this.species = TreeManager.treeFactory.createSpecies(uid, unlocalizedName, "Sengir", unlocalizedDescription, dominant, branch.getBranch(), binomial, uid.substring("forestry.".length()), leafIconProvider, germlingModelProvider, this);
		this.woodType = null;
