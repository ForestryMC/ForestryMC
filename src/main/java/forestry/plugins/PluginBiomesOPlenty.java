...
public class PluginBiomesOPlenty extends ForestryPlugin {
	...
	private static void addFarmCrops() {
		...
		for (String key : saplingItemKeys) {
			...
			FMLInterModComms.sendMessage(Defaults.MOD, "add-farmable-sapling",
		...
		if (boPTurnip != null) {
			...
			if (boPTurnipSeeds != null) {
				Farmables.farmables.get("farmVegetables")
						.add(new FarmableGenericCrop(new ItemStack(boPTurnipSeeds, 1, 0), boPTurnip, 7));
			}
	...
	private static void addFermenterRecipes() {
		...
		if (saplings != null) {
			RecipeUtil.injectLeveledRecipe(new ItemStack(saplings, 1, OreDictionary.WILDCARD_VALUE), saplingYield,
					Fluids.BIOMASS);
		}

		if (colorizedSaplings != null) {
			RecipeUtil.injectLeveledRecipe(new ItemStack(colorizedSaplings, 1, OreDictionary.WILDCARD_VALUE),
					saplingYield, Fluids.BIOMASS);
		}
	}

	private static void addSqueezerRecipes() {
		if (food != null) {
			RecipeManagers.squeezerManager.addRecipe(10, new ItemStack[] { new ItemStack(food) },
					Fluids.JUICE.getFluid(50), ForestryItem.mulch.getItemStack(), 5);
		}

		if (persimmon != null) {
			RecipeManagers.squeezerManager.addRecipe(10, new ItemStack[] { persimmon }, Fluids.JUICE.getFluid(200),
					ForestryItem.mulch.getItemStack(), 20);
		}
	}
	...
	private static void addFlowers() {
		...
		if (flowers != null) {
			...
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 1, 1.0, FlowerManager.FlowerTypeVanilla,
					FlowerManager.FlowerTypeMushrooms); // Swampflower
			// FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 2,
			// 1.0, FlowerManager.FlowerTypeNether); //Deathbloom
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 3, 1.0, FlowerManager.FlowerTypeVanilla); // GlowFlower
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 4, 1.0, FlowerManager.FlowerTypeVanilla); // Blue
																													// Hydrangea
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 5, 1.0, FlowerManager.FlowerTypeVanilla,
					FlowerManager.FlowerTypeJungle); // Orange Cosmos
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 6, 1.0, FlowerManager.FlowerTypeVanilla); // Pink
																													// Daffodil
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 7, 1.0, FlowerManager.FlowerTypeVanilla); // WildFlower
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 8, 1.0, FlowerManager.FlowerTypeVanilla,
					FlowerManager.FlowerTypeSnow); // Violet
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 9, 1.0, FlowerManager.FlowerTypeVanilla); // White
																													// Anemone
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 10, 1.0, FlowerManager.FlowerTypeVanilla); // Waterlily
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 11, 1.0, FlowerManager.FlowerTypeVanilla); // EnderLotus
																														// (does
																														// not
																														// actually
																														// spawn
			...
			// FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 14,
			// 1.0, FlowerManager.FlowerTypeNether); // Unlisted top of the
			// eyebulb
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers, 15, 1.0, FlowerManager.FlowerTypeVanilla); // Dandelion
																														// Puff
		}

		Block flowers2 = GameRegistry.findBlock(BoP, "flowers2");
		if (flowers2 != null) {
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers2, 0, 1.0, FlowerManager.FlowerTypeVanilla,
			...
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers2, 6, 1.0, FlowerManager.FlowerTypeVanilla); // Miner's
																														// delight
			FlowerManager.flowerRegistry.registerPlantableFlower(flowers2, 7, 1.0, FlowerManager.FlowerTypeVanilla,
		...
		// Toadstool 0 Portobello 1 Blue Milk cap 2 Glowshroom 3 flat mushroom 4
		// shadow shroom 5 'plants:12' tiny cactus
		Block mushrooms = GameRegistry.findBlock(BoP, "mushrooms");
		if (mushrooms != null) {
			FlowerManager.flowerRegistry.registerPlantableFlower(mushrooms, OreDictionary.WILDCARD_VALUE, 1.0,
	...
	protected void registerBackpackItems() {
		...
		addBlocksToBackpack(DIGGER, "bopGrass", "newBopGrass", "longGrass", "mud", "hardDirt", "hardSand",
				"originGrass", "ash", "newBopDirt");
