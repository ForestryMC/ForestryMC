...
@Plugin(
        pluginID = "MinefactoryReloaded",
        name = "MineFactoryReloaded",
        author = "Nirek",
        url = Constants.URL,
        unlocalizedDescription = "for.plugin.minefactoryreloaded.description")
        ...
        if (PluginManager.Module.APICULTURE.isEnabled() && !PluginManager.Module.INDUSTRIALCRAFT.isEnabled()) {
            if (rubberRaw != null) {
                ...
                RecipeManagers.centrifugeManager
                        .addRecipe(20, beeItems.propolis.get(EnumPropolis.NORMAL, 1), ImmutableMap.of(rubberRaw, 1.0f));
            }
        }
