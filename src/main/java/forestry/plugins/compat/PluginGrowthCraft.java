...
@Plugin(
        pluginID = "Growthcraft",
        name = "Growthcraft",
        author = "Nirek",
        url = Constants.URL,
        unlocalizedDescription = "for.plugin.growthcraft.description")
    ...
    protected void registerRecipes() {
        ...
        if (appleSeed != null) {
            RecipeManagers.squeezerManager
                    .addRecipe(10, new ItemStack[] { appleSeed }, Fluids.SEEDOIL.getFluid(seedamount));
            BackpackManager.backpackItems[2].add(appleSeed);
        }
        if (grapeSeed != null) {
            RecipeManagers.squeezerManager
                    .addRecipe(10, new ItemStack[] { grapeSeed }, Fluids.SEEDOIL.getFluid(seedamount));
            BackpackManager.backpackItems[2].add(grapeSeed);
        }
        if (hopSeed != null) {
            RecipeManagers.squeezerManager
                    .addRecipe(10, new ItemStack[] { hopSeed }, Fluids.SEEDOIL.getFluid(seedamount));
            BackpackManager.backpackItems[2].add(hopSeed);
        }
        if (riceSeed != null) {
            RecipeManagers.squeezerManager
                    .addRecipe(10, new ItemStack[] { riceSeed }, Fluids.SEEDOIL.getFluid(seedamount));
        ...
        if (hopVine != null) {
            Farmables.farmables.get(FarmableReference.Orchard.get())
                    .add(new FarmableBasicGrowthCraft(hopVine, 3, false, false)); // need custom fruit to weed out rope
        }
        Block riceBlock = GameRegistry.findBlock("Growthcraft|Rice", "grc.riceBlock");
        if (riceBlock != null) {
            Farmables.farmables.get(FarmableReference.Orchard.get())
                    .add(new FarmableBasicGrowthCraft(riceBlock, 7, true, false)); // need to set paddyfield to 7
        }
        ...
        if (PluginManager.Module.APICULTURE.isEnabled()) {
            ...
            if (comb != null) {
                ...
                RecipeManagers.centrifugeManager
                        .addRecipe(20, emptyComb, ImmutableMap.of(PluginCore.items.beeswax.getItemStack(), 1.0f));
                ...
                RecipeManagers.centrifugeManager.addRecipe(
                        ...
                        ImmutableMap.of(
                                ...
                                1.0f,
                                PluginApiculture.items.honeyDrop.getItemStack(),
                                0.9f,
                                PluginApiculture.items.honeydew.getItemStack(),
                                0.1f));
            }
        }
        ...
        if (grapes.getItem() != null) {
            RecipeManagers.squeezerManager
                    .addRecipe(10, new ItemStack[] { grapes }, Fluids.JUICE.getFluid(juiceAmount));
            BackpackManager.backpackItems[2].add(grapes);
        }
        Block grapeBlock = GameRegistry.findBlock("Growthcraft|Grapes", "grc.grapeBlock");
        if (grapeBlock != null) {
            Farmables.farmables.get(FarmableReference.Orchard.get())
                    .add(new FarmableBasicGrowthCraft(grapeBlock, 0, false, true));
        }
