...
public class PluginPlantMegaPack extends ForestryPlugin {
        ...
        for (String reedLike : reedLikePlant) {
            ...
            if (reedBlock != null && reedStack != null) {
                RecipeUtil.addFermenterRecipes(
                        reedStack, ForestryAPI.activeMode.getIntegerSetting("fermenter.yield.wheat"), Fluids.BIOMASS);
            ...
            if (landCropBlock != null) {
                if (foodStack != null) {
                    RecipeManagers.squeezerManager.addRecipe(
                            10, new ItemStack[] {foodStack}, Fluids.JUICE.getFluid(juiceAmount));
                }
                if (seedStack != null) {
                    RecipeManagers.squeezerManager.addRecipe(
                            10, new ItemStack[] {seedStack}, Fluids.SEEDOIL.getFluid(seedamount));
                }
                if (PluginManager.Module.FARMING.isEnabled()) {
                    Farmables.farmables
            ...
            if (flowerPlantBlock != null) {
                for (int i = 0; i < flower.getValue(); i++) {
                    FlowerManager.flowerRegistry.registerPlantableFlower(
                            flowerPlantBlock, i, 0.75, FlowerManager.FlowerTypeVanilla);
                }
            }
        }
    ...
    private static void addMetaFlower(
            ImmutableMap<String, Integer> flowerMap, boolean plantable, String... flowertype) {
