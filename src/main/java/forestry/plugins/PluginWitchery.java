...
public class PluginWitchery extends ForestryPlugin {
	...
	protected void registerRecipes() {
		...
		RecipeUtil.injectLeveledRecipe(saplingStack, GameMode.getGameMode().getIntegerSetting("fermenter.yield.wheat"),
				Fluids.BIOMASS);
		...
		for (String cropDirectName : cropDirect) {
			...
			ItemStack cropDirectStack = new ItemStack(GameRegistry.findItem(Witch, cropDirectName), 1);
		...
		for (Map.Entry<String, Integer> cropSeedName : cropSeed.entrySet()) {
			...
			ItemStack cropSeedStack = new ItemStack(GameRegistry.findItem(Witch, "seeds" + cropSeedName.getKey()), 1);
			if (cropSeedStack != null && cropSeedBlock != null) {
				RecipeManagers.squeezerManager.addRecipe(10, new ItemStack[] { cropSeedStack },
