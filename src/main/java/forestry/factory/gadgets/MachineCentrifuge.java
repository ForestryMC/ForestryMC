...
public class MachineCentrifuge extends TilePowered implements ISocketable, ISidedInventory {
	...
	public void openGui(EntityPlayer player) {
		player.openGui(ForestryAPI.instance, GuiId.CentrifugeGUI.ordinal(), player.worldObj, pos.getX(), pos.getY(),
				pos.getZ());
	}
	...
	public boolean hasResourcesMin(float percentage) {
		...
		return ((float) inventory.getStackInSlot(SLOT_RESOURCE).stackSize
				/ (float) inventory.getStackInSlot(SLOT_RESOURCE).getMaxStackSize()) > percentage;
	}
	...
	public static class RecipeManager implements ICentrifugeManager {
		...
		public Map<Object[], Object[]> getRecipes() {
			...
			for (ICentrifugeRecipe recipe : recipes) {
				...
				recipeList.put(new Object[] { recipe.getInput() },
						productsKeys.toArray(new ItemStack[productsKeys.size()]));
			}
