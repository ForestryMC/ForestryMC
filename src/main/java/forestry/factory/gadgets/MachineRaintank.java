...
public class MachineRaintank extends TileBase implements ISidedInventory, ILiquidTankContainer {
	...
	public void openGui(EntityPlayer player) {
		player.openGui(ForestryAPI.instance, GuiId.RaintankGUI.ordinal(), player.worldObj, pos.getX(), pos.getY(),
				pos.getZ());
	}
	...
	public void updateServerSide() {
		...
		if (!isFilling()) {
			...
			fillingTime--;
			if (fillingTime <= 0 && FluidHelper.fillContainers(tankManager, this, SLOT_RESOURCE, SLOT_PRODUCT,
	...
	private void tryToStartFillling() {
		...
		if (!FluidHelper.fillContainers(tankManager, getInternalInventory(), SLOT_RESOURCE, SLOT_PRODUCT,
				Fluids.WATER.getFluid(), false)) {
			return;
		}
