...
public class PositionedFluidTank {
	...
	public List<String> handleTooltip(List<String> currenttip) {
		if (this.tank == null || this.tank.getFluid() == null || this.tank.getFluid().getFluid() == null
				|| this.tank.getFluid().amount <= 0) {
			return currenttip;
		}
		currenttip.add(this.tank.getFluid().getLocalizedName());
		if (this.showAmount) {
			currenttip.add(EnumChatFormatting.GRAY.toString() + this.tank.getFluid().amount
	...
	public void draw() {
		...
		int amount = Math.max(Math.min(this.position.height,
				this.tank.getFluid().amount * this.position.height / this.tank.getCapacity()), 1);
		...
		for (int i = 0; i < this.position.width; i += 16) {
			for (int j = 0; j < amount; j += 16) {
				...
				TextureAtlasSprite sprite = Minecraft.getMinecraft().getTextureMapBlocks()
						.getAtlasSprite(fluidIcon.toString());
				...
				worldRenderer.addVertexWithUV(drawX, drawY + drawHeight, 0, minU,
						minV + (maxV - minV) * drawHeight / 16F);
				worldRenderer.addVertexWithUV(drawX + drawWidth, drawY + drawHeight, 0,
						minU + (maxU - minU) * drawWidth / 16F, minV + (maxV - minV) * drawHeight / 16F);
				worldRenderer.addVertexWithUV(drawX + drawWidth, drawY, 0, minU + (maxU - minU) * drawWidth / 16F,
						minV);
		...
		if (this.overlayTexture != null && this.overlayTexturePos != null) {
			...
			GuiDraw.drawTexturedModalRect(this.position.x, this.position.y, this.overlayTexturePos.x,
					this.overlayTexturePos.y, this.position.width, this.position.height);
		}
	}
}
