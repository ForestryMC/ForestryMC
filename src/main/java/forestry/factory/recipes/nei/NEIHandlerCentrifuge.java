...
public class NEIHandlerCentrifuge extends RecipeHandlerBase {
	...
	public class CachedCentrifugeRecipe extends CachedBaseRecipe {
		...
		public void setResults(Map<ItemStack, Float> outputs) {
			...
			PriorityQueue<Entry<ItemStack, Float>> sortByChance = new PriorityQueue<Entry<ItemStack, Float>>(
					entrySet.size(), highestChanceComparator);
			...
			while (!sortByChance.isEmpty()) {
				...
				PositionedStackAdv output = new PositionedStackAdv(stack.getKey(), 93 + OUTPUTS[i][0] * 18,
						8 + OUTPUTS[i][1] * 18);
