	...
	public class CachedFermenterRecipe extends CachedBaseRecipe implements INBTMatchingCachedRecipe {
		...
		public CachedFermenterRecipe(MachineFermenter.Recipe recipe, ItemStack fermentable, boolean genPerms) {
			if (recipe.liquid != null) {
				...
				this.tanks.add(new PositionedFluidTank(input, 10000, new Rectangle(30, 4, 16, 58),
						NEIHandlerFermenter.this.getGuiTexture(), new Point(176, 0)));
			}
			if (recipe.output != null) {
				...
				if (fermentable.getItem() instanceof IVariableFermentable) {
					output.amount = (int) (recipe.fermentationValue * recipe.modifier
							* ((IVariableFermentable) fermentable.getItem()).getFermentationModifier(fermentable));
				} else {
					output.amount = (int) (recipe.fermentationValue * recipe.modifier);
				}
				this.tanks.add(new PositionedFluidTank(output, 10000, new Rectangle(120, 4, 16, 58),
						NEIHandlerFermenter.this.getGuiTexture(), new Point(176, 0)));
			}
