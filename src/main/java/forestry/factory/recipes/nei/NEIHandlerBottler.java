...
public class NEIHandlerBottler extends RecipeHandlerBase {
	...
	public void prepare() {
		for (FluidContainerData container : FluidContainerRegistry.getRegisteredFluidContainerData()) {
			MachineBottler.Recipe recipe = MachineBottler.RecipeManager.findMatchingRecipe(container.fluid,
	...
	public class CachedBottlerRecipe extends CachedBaseRecipe {
		...
		public CachedBottlerRecipe(MachineBottler.Recipe recipe) {
			if (recipe.input != null) {
				this.fluid = new PositionedFluidTank(recipe.input, 10000, new Rectangle(48, 6, 16, 58),
						NEIHandlerBottler.this.getGuiTexture(), new Point(176, 0));
			}
			if (recipe.can != null) {
