...
public class NEIHandlerMoistener extends RecipeHandlerBase {
	...
	public class CachedMoistenerRecipe extends CachedBaseRecipe {
		...
		public CachedMoistenerRecipe(MachineMoistener.Recipe recipe, MoistenerFuel fuel) {
			this.tank = new PositionedFluidTank(FluidRegistry.getFluidStack("water", 10000), 10000,
					new Rectangle(11, 5, 16, 58), NEIHandlerMoistener.this.getGuiTexture(), new Point(176, 0));
	...
	public void loadCraftingRecipes(ItemStack result) {
		for (MachineMoistener.Recipe recipe : MachineMoistener.RecipeManager.recipes) {
			for (MoistenerFuel fuel : FuelManager.moistenerResource.values()) {
				if (NEIServerUtils.areStacksSameTypeCrafting(recipe.product, result)
