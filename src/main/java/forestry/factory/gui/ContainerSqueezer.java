...
public class ContainerSqueezer extends ContainerLiquidTanks {
	...
	public ContainerSqueezer(InventoryPlayer player, MachineSqueezer tile) {
		...
		for (int l = 0; l < 3; l++)
			for (int k = 0; k < 3; k++)
				addSlotToContainer(new Slot(tile, k + l * 3, 19 + k * 18, 18 + l * 18));

		// Remnants slot
		this.addSlotToContainer(new SlotClosed(tile, 9, 123, 19));

		// Can slot
		this.addSlotToContainer(new SlotLiquidContainer(tile, 10, 106, 55, true));
		// Output slot
		this.addSlotToContainer(new SlotClosed(tile, 11, 140, 55));
