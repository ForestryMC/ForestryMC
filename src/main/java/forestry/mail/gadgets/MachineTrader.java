...
public class MachineTrader extends TileBase {
	...
	public void openGui(EntityPlayer player) {
		if (isLinked()) {
			player.openGui(ForestryAPI.instance, GuiId.TraderGUI.ordinal(), worldObj, pos.getX(), pos.getY(),
					pos.getZ());
		} else {
			player.openGui(ForestryAPI.instance, GuiId.TraderNameGUI.ordinal(), worldObj, pos.getX(), pos.getY(),
					pos.getZ());
		}
	}
	...
	public void updateServerSide() {
		...
		if (tradeGood != null) {
			boolean hasSupplies = hasItemCount(TradeStation.SLOT_SEND_BUFFER, TradeStation.SLOT_SEND_BUFFER_COUNT,
					tradeGood, tradeGood.stackSize);
	...
	/**
	 * Returns true if there are 'itemCount' of 'item' in the inventory wildcard
	 * when item == null, counts all types of items
	 */
	...
	/**
	 * Returns the percentage of the inventory that is occupied by 'item' if
	 * item == null, returns the percentage occupied by all kinds of items
	 */
	...
	public boolean hasPaperMin(int count) {
		return hasItemCount(TradeStation.SLOT_LETTERS_1, TradeStation.SLOT_LETTERS_COUNT, new ItemStack(Items.paper),
				count);
	}
	...
	public boolean hasInputBufMin(float percentage) {
		...
		return percentOccupied(TradeStation.SLOT_SEND_BUFFER, TradeStation.SLOT_SEND_BUFFER_COUNT,
				tradeGood) > percentage;
	}

	public boolean hasOutputBufMin(float percentage) {
		return percentOccupied(TradeStation.SLOT_RECEIVE_BUFFER, TradeStation.SLOT_RECEIVE_BUFFER_COUNT,
				null) > percentage;
	}
	...
	public IInventoryAdapter getInternalInventory() {
		...
		return (TradeStation) PostManager.postRegistry.getOrCreateTradeStation(worldObj, getAccessHandler().getOwner(),
				address);
	}
