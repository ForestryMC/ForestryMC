...
public class GuiLetter extends GuiForestry<ContainerLetter, ItemInventoryLetter> {
    ...
    private void drawTradePreview(int x, int y) {
        ...
        if (infoString != null) {
            fontRendererObj.drawSplitString(
                    StringUtil.localize(infoString),
                    guiLeft + x,
                    guiTop + y,
                    119,
                    fontColor.get("gui.mail.lettertext"));
        ...
        fontRendererObj.drawString(
                StringUtil.localize("gui.mail.pleasesend"),
                guiLeft + x,
                guiTop + y,
                fontColor.get("gui.mail.lettertext"));
        ...
        fontRendererObj.drawString(
                StringUtil.localize("gui.mail.foreveryattached"),
                guiLeft + x,
                guiTop + y + 28,
                fontColor.get("gui.mail.lettertext"));
        ...
        for (int i = 0; i < container.getTradeInfo().required.length; i++) {
            addTradeInfoWidget(
                    new ItemStackWidget(widgetManager, x + i * 18, y + 38, container.getTradeInfo().required[i]));
        }
    }
