...
public class GuiCatalogue extends GuiForestry<ContainerCatalogue> {
	...
	public void init() {
		...
		addButton(new Button(width / 2 + 44, guiTop + 150, 12, 20, new StringTextComponent(">"), b -> actionPerformed(2)));
		addButton(new Button(width / 2 - 58, guiTop + 150, 12, 20, new StringTextComponent("<"), b -> actionPerformed(3)));
	...
	private void drawTradePreview(MatrixStack transform, ITradeStationInfo tradeInfo, int x, int y) {
		...
		fontRenderer.func_243248_b(transform, new TranslationTextComponent("for.gui.mail.willtrade", tradeInfo.getOwner().getName()), x, y + 18, ColourProperties.INSTANCE.get("gui.book"));

		addTradeInfoWidget(new ItemStackWidget(widgetManager, x - guiLeft, y - guiTop + 28, tradeInfo.getTradegood()));

		fontRenderer.func_243248_b(transform, new TranslationTextComponent("for.gui.mail.tradefor"), x, y + 46, ColourProperties.INSTANCE.get("gui.book"));
