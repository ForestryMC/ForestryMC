...
public class GuiCatalogue extends GuiForestry<ContainerCatalogue, IInventory> {
	...
	public void initGui() {
		...
		buttonList.add(buttonFilter = new GuiButton(4, (width / 2) - 44, guiTop + 150, 42, 20,
				StringUtil.localize("gui.mail.filter.all")));

		buttonList.add(buttonUse = new GuiButton(5, width / 2, guiTop + 150, 42, 20,
				StringUtil.localize("gui.mail.address.copy")));
	}
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int mouseX, int mouseY) {
		...
		fontRendererObj.drawString(String.format("%s / %s", container.getPageNumber(), container.getPageCount()),
				guiLeft + xSize - 72, guiTop + 12, fontColor.get("gui.book"));
	...
	private void drawNoTrade(int x, int y) {
		fontRendererObj.drawSplitString(StringUtil.localize("gui.mail.notrades"), x, y + 18, 119,
	...
	private void drawTradePreview(TradeStationInfo tradeInfo, int x, int y) {
		...
		fontRendererObj.drawString(String.format(StringUtil.localize("gui.mail.willtrade"), tradeInfo.owner.getName()),
				x, y + 18, fontColor.get("gui.book"));
		...
		if (tradeInfo.state.isOk()) {
			fontRendererObj.drawSplitString(
					...
					x, y + 82, 119, fontColor.get("gui.book"));
		} else {
			fontRendererObj.drawSplitString(
