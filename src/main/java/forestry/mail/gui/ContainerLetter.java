...
public class ContainerLetter extends ContainerItemInventory<LetterInventory> {
	...
	public ContainerLetter(EntityPlayer player, LetterInventory inventory) {
		...
		for (int i = 0; i < 4; i++) {
			addSlotToContainer(
					new SlotFiltered(inventory, Letter.SLOT_POSTAGE_1 + i, 150, 14 + i * 19).setStackLimit(1));
		}
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 9; j++) {
				addSlotToContainer(
						new SlotFiltered(inventory, Letter.SLOT_ATTACHMENT_1 + j + i * 9, 17 + j * 18, 98 + i * 18));
			}
		}
	...
	private static IMailAddress getRecipient(String recipientName, EnumAddressee type) {
		...
		case PLAYER: {
			GameProfile gameProfile = MinecraftServer.getServer().getPlayerProfileCache()
					.getGameProfileForUsername(recipientName);
			if (gameProfile == null) {
