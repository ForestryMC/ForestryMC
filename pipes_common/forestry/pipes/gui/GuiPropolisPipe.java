...
public class GuiPropolisPipe extends GuiForestry {
	...
	class SpeciesFilterSlot extends Widget {
		...
		public void draw(int startX, int startY) {
			...
			for (int i = 0; i < 3; ++i) {
				...
				Icon icon = ((ItemBeeGE) ForestryItem.beeDroneGE.item()).getIconFromSpecies((IAlleleBeeSpecies) species, i);
	...
	public GuiPropolisPipe(EntityPlayer player, PipeItemsPropolis pipe) {
		...
		IApiaristTracker tracker = PluginApiculture.beeInterface.getBreedingTracker(player.worldObj, player.username);
		for (int i = 0; i < 6; i++)
			for (int j = 0; j < 3; j++)
				for (int k = 0; k < 2; k++)
